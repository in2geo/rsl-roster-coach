...
| **Great Hall** | ❌ **NOT exported by Gestal** | ⚠️ estimated | `estimate-stats.js` substitutes an `account_development` bundle. ❓ real values readable in-game. Game text calls Faction Guardians "similar to Great Hall Bonuses" — they are the same FAMILY: **account-level global stat bonuses, active in every battle**, of which we capture NONE properly (Great Hall, Faction Guardians, Arena all collapse into one poor/fair/good guess) |
