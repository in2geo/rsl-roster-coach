...
| **Great Hall** (Affinity Bonuses) | ❌ levels NOT exported by Gestal | ⚠️ **MODELLED BACKWARDS** | **VERIFIED IN-GAME 2026-07-21.** Per-AFFINITY (Magic/Force/Spirit/Void) x 6 stats (HP/ATK/DEF/C.DMG/RES/ACC), levels 0-10, **applied as a PERCENTAGE OF BASE STATS**. Curve: +2/3/4/6/8/10/12/14/17/20%. Data: `data/great-hall-bonus-stats.json`.<br>⚠⚠ **OUR CODE HAS IT INVERTED.** `applyAccountBonus()` adds FLAT ACC +40 / RES +30 and deliberately OMITS HP/ATK/DEF, claiming Great Hall HP/ATK/DEF are 'flat in-game (not % of base)'. The game says the opposite. Measured on our own 944 champions: **604 have base_acc = 0** (non-zero avg 14), so +20% ACC is worth ~0-3, NOT 40 — while base HP averages 17,439, so the +20% HP we discard is worth ~3,488. We invent ~37 ACC (≈3.7 Spider stages of the stage x 10 floor) and throw away the part that matters.<br>Also note `estimate-stats.js` contradicts ITSELF: line ~17 says Gestal effective_stats 'already includes Great Hall/Arena' (so do not double-count), line ~173 says Gestal does NOT export it (so add). `applyAccountBonus` IS called on the Gestal path (match-engine.js:306), so the second branch wins — unreconciled. |
