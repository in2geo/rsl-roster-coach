...
- Ad-hoc measurements this session were throwaway scripts in the repo root (`_measure-*.mjs`, deleted). Pattern: `import {makeState,simulate} from './lib/sim/engine.js'` + `buildBattle/applyBattleLayers` from `dragon-fixture.js`, loop seeds, read `res.effects` (kind: damage/dot/activate/evade/counterattack, with `phase`) and `res.log` (death/revive/INHALE/SCORCH/... events). `state.onAction=(st,actor,skill)=>{}` hook (engine.js:1321) for per-decision instrumentation. Fixture: `test/golden/dragon-donahilvi-ezio-xeno-st20-current.json`.
- 🚨 **A custom seed loop MUST call `installRecipeRun(st)` after `makeState` and before `simulate`** (as `sim-per-hero-bands.mjs:146` does). Without it, `dispatchSkill` falls back to the registry (basic recipes still run, so it LOOKS fine) but the state hooks are unset: **no `onDamageTaken` (boss counterattacks / on-hit shields never fire), no `onRoundStart` (Ezio Perfect Veil), no passive-cooldown tick, no on-death triggers.** This session's ad-hoc WR numbers omitted it → ran a degraded sim, understating WR ~2pp and falsely zeroing boss counterattacks. Ezio's counterattack IS modeled and firing (~1.00/run on the boss, A1 re-run at 35% of boss hits) — only the degraded measurement hid it.
