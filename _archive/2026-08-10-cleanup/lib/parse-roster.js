...
import { buildNameResolver } from './champion-names.js';   // resolve an OCR'd display name → champions.id (aliases included), never a raw lowercase name match
...
// The ONE name registry (champions.name + champion_aliases) so an OCR'd display name resolves to the
// champion by ID — an alias/variant lands instead of silently missing on a lowercase equality check.
...
async function getResolver() {
  ...
  resolverCache = { resolver: buildNameResolver(champs, aliases || []), byId: new Map(champs.map(c => [c.id, c])) };
...
export async function parseRosterScreenshot(imageBase64, mediaType) {
  ...
  const { resolver, byId } = await getResolver();   // name→champions.id registry for the OCR-name match below
  ...
  for (const card of cards) {
    ...
    const hit = resolver.resolve(card.name);
    const match = hit ? byId.get(hit.id) : null;
