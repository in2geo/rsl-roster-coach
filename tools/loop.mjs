...
for (const b of (Array.isArray(log) ? log : [])) {
  ...
  measure.push({ account: b.displayName ?? b.accountId, content: contentKey, isCB, won, classification,
...
const accuracy = `# Model Accuracy — self-populating (generated by tools/loop.mjs)\n\n`
  ...
  + `_Caveat: "agreement" is partly circular where a player fields near the coverage recommendation. The\n`
  + `per-account and win/loss splits are what disentangle a real signal from that — read those, not the totals._\n`;
