...
// ── THE SCALAR ───────────────────────────────────────────────────────────────
// Everything else in this file is a DESCRIPTION — calibration bands, agreement averages, bench/field
// counts. Descriptions can always be argued with, which is the diagnosed cause of the zigzag
// (MODEL_AS_REIMPLEMENTATION.md). battle-suite is the one falsifiable number, so it goes at the TOP
// of the evidence doc, not in a tool someone has to remember to run.
let suiteLine = '_(battle-suite did not run — check SUPABASE env)_';
try {
  const r = spawnSync(process.execPath, [path.join(__dirname, 'battle-suite.mjs'), '--brief'],
    { cwd: REPO, encoding: 'utf8', env: process.env });
  const out = String(r.stdout ?? '').trim().split(/\r?\n/).filter(Boolean).pop();
  if (out) suiteLine = out;
} catch { /* the loop's own output must survive a suite failure */ }
console.log(suiteLine);

const accuracy = `# Model Accuracy — self-populating (generated by tools/loop.mjs)\n\n`
  + `## THE NUMBER — \`tools/battle-suite.mjs\`\n\n`
  + `\`\`\`\n${suiteLine}\n\`\`\`\n\n`
  + `Balanced accuracy over every captured battle replayed as a WIN/LOSS test case; a constant\n`
  + `predictor scores 50.0%. **This is the only falsifiable line in this document** — everything\n`
  + `below describes the model's behaviour, which is useful for diagnosis but cannot settle whether\n`
  + `a change made the model better. See \`knowledge/MODEL_AS_REIMPLEMENTATION.md\`.\n\n`
  + `---\n\n`
