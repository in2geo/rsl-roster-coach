...
if (!fs.existsSync(FIX)) { softNotes.push(`no fixture dragon${STAGE}-donbambus-current.json — L3 skipped`); say('    ⚠ no fixture — L3 skipped'); }
  ...
  const BENIGN = [/immune/, /^resisted /, /^overheal/, /^no debuffs to cleanse$/, /^no buffs to steal$/, /^no buffs to reduce$/, /^no buffs to extend$/, /^no \[Shield\] to boost$/, /^no buffs decreased/, /^no cooldowns to decrease$/, /^nothing to /, /^turn lost$/, /^proc missed /, /^missed placement /, /^missed \(/, /^already full$/, /^already empty$/, /^condition not met$/, /^nullified /, /^fully absorbed/];
