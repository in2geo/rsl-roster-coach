...
//   'reactive' — [Shield]/[Magma Shield]/[Reflect Damage] must produce an 'absorb'/'reflect' WHEN the buffed
//                unit is hit (opportunity = it actually took a hit; a shield on a never-hit ally is not inert).
...
export function scoreOutcomes(fx, actedAt) {
  ...
  const placements = {}, conseq = {}, hitAt = {}, firedNotConsumed = [];
  ...
  for (const e of fx) {
    ...
    else if (e.kind === 'absorb' || e.kind === 'reflect') bump(e.subtype);                    // [Shield]/[Magma Shield]/[Reflect Damage] → soak/reflect
