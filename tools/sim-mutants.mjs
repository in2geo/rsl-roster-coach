...
const MUTANTS = [
  { name: 'def-mitigation-noop (DEF ignored entirely)', expectKill: true,
    find: 'export const defMitigation = (def, level = 60) => 1 - 0.85 * (1 - Math.exp(-2 * Math.max(0, def ?? 0) / (50 * (level || 60))));',
    repl: 'export const defMitigation = (def, level = 60) => 1 - 0.85 * (1 - Math.exp(-2 * Math.max(0, 0) / (50 * (level || 60))));' },
  { name: 'decrease-DEF does nothing (statFactor drops the Decrease-DEF shred)', expectKill: true,
    find: 'const down = Math.min(1, best(m.down, c.debuffs));',
    repl: 'const down = 0 * Math.min(1, best(m.down, c.debuffs));' },
  { name: 'land-chance swaps ACC and RES', expectKill: true,
    find: 'const x = (res - acc) / 100;',
    repl: 'const x = (acc - res) / 100;' },
  // COVERAGE GAP (probe): the engine's [Continuous Heal] HoT (tickHots) overheal is NOT pinned by any no-DB
  // sim rung — sim-invariants doesn't place a HoT on a full-HP combatant. The MODEL side DOES catch its analog
  ...
  { name: 'targeting inverts the lowest-current-HP% rule (probe: sim side has no targeting-coverage rung)', expectKill: false,
