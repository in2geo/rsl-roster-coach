...
export function findBattleGaps(evaluation, battle, chestTiers = null, team = null) {
  ...
  // ── 2b. DEBUFF EFFECTIVENESS (Layer 1 guardrail — damage-mechanics.js) ───────
  // Catch "right tag, wrong reason": a fielded damage-debuff that (a) can't multiply
  // this team's damage TYPE (Dec DEF on a poison team), or (b) is too unreliable to
  // lean on (low land-chance × uptime). Where reliability inputs aren't captured, it
  // emits a tracked data_missing gap instead of silently full-crediting the debuff.
  for (const f of auditTeamDebuffs(team ?? [], { fightTurns: battle.turns ?? null })) {
    if (f.kind === 'damage_type_mismatch')
      add('contradiction', 'debuff_type_mismatch', `${f.champion}'s ${f.tag} adds ~nothing to this team's damage`, f.detail,
          'attribute this champion\'s value to its OTHER roles (sustain/CC/heal), not the mismatched debuff — and prefer a debuff that matches the team\'s damage type. NOT a coverage gap.');
    else if (f.kind === 'low_reliability')
      add('contradiction', 'debuff_low_reliability', `${f.champion}'s ${f.tag} is unreliable for its role`, f.detail,
          'surface the reliability caveat in the explanation; prefer a higher land-chance / lower-cooldown option for this role');
    else if (f.kind === 'reliability_unknown')
      add('data_missing', 'debuff_reliability', `Debuff reliability not assessable: ${f.champion}'s ${f.tag}`, f.detail,
          'capture debuff chance_unbooked + duration + auto_reliable so reliability × uptime can be scored (see the Layer-1 data gap)');
  }
