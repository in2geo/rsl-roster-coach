...
import { sustainContribution, contentThreatWeight, controlStrength, ccEffectiveness } from './sustain-profiles.js';
...
const W_GRANT = 0.6;
// Crowd control is a survival axis (denies enemy turns, INS-0004), co-equal with sustain but
// scaled per-content by CC-effectiveness (immunity/adds) so it's ~free on CC-immune content.
const W_CONTROL_BASE = 1.0;
...
export function computeCompositeScores(roster = [], { contentKey, teamSources = [] } = {}) {
  ...
  // CC-effectiveness is applied as a WEIGHT (not inside the normalized value) so per-content
  // CC-immunity survives normalization — otherwise a constant per-content factor cancels out.
  const wControl = W_CONTROL_BASE * ccEffectiveness(contentKey);
  const raw = roster.map(c => {
    const sc = sustainContribution(c.tags ?? [], contentKey);
    return { champ: c, name: c.name, dmg: rawDamage(c), sus: sc.score, grant: rawGrant(c, teamSources),
      ctrl: controlStrength(c.tags ?? []), sustainMechanisms: sc.mechanisms };
  });
  ...
  const maxCtrl  = Math.max(0, ...raw.map(r => r.ctrl));
  return raw.map(r => {
    const damage  = normBy(r.dmg, maxDmg);
    const sustain = normBy(r.sus, maxSus);
    const grant   = normBy(r.grant, maxGrant);
    const control = normBy(r.ctrl, maxCtrl);
    const composite = W_DAMAGE * damage + wSustain * sustain + W_GRANT * grant + wControl * control;
    return { name: r.name, composite, damage, sustain, grant, control, sustainMechanisms: r.sustainMechanisms, champ: r.champ };
  });
}
...
function dominantRole(score, { teamSources }) {
  const parts = [
    ...
    { k: 'control', w: score.control ?? 0,       label: 'crowd control (denies enemy turns)' },
  ].sort((a, b) => b.w - a.w);
...
export function runWatchdog({ roster = [], team = [], contentKey, usabilityTier = () => 0, fightTurns = null } = {}) {
  ...
  // roster/team here are mapRoster().mapped objects, which carry champions.id — key identity
  // by id, never by name (CLAUDE.md: never look a champion up by a raw name). `.name` stays
  // for human-readable flag output only.
  const byId = new Map(scores.map(s => [s.champ.id, s]));
  const fieldedIds = new Set(team.map(c => c.id));

  const fielded = team.map(c => ({ champ: c, score: byId.get(c.id), tier: usabilityTier(c) }))
    .filter(x => x.score);
  const benched = roster.filter(c => !fieldedIds.has(c.id))
    .map(c => ({ champ: c, score: byId.get(c.id), tier: usabilityTier(c) }))
    .filter(x => x.score);
  ...
  return {
    ...
    reliability,
    scores: scores
      .map(s => ({ name: s.name, composite: round2(s.composite), damage: round2(s.damage), sustain: round2(s.sustain), grant: round2(s.grant), control: round2(s.control ?? 0), fielded: fieldedIds.has(s.champ.id) }))
