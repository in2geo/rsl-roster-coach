...
export function runWatchdog({ roster = [], team = [], contentKey, usabilityTier = () => 0, fightTurns = null } = {}) {
  ...
  return {
    ...
    reliability,
    scores: scores
      .map(s => ({ name: s.name, composite: round2(s.composite), damage: round2(s.damage), sustain: round2(s.sustain), grant: round2(s.grant), control: round2(s.control ?? 0), fielded: fieldedNames.has(s.name) }))
