...
const CHAMPION_SELECT = `
  id, name, type_id, rarity, role, affinity, faction,
  base_hp, base_atk, base_def, base_spd, base_acc, base_res,
  base_crit_rate, base_crit_dmg,
  champion_tags ( tag_id, status, ascension_required, tags ( name, is_debuff, bypasses_accuracy_check ) )
`;
...
export async function buildRosterMapper(roster, supabase) {
  ...
  // Load the whole alias registry (cheap, no join) so a roster name in ANY form —
  // long, short, apostrophe — resolves. Find aliases whose normalized text matches an
  // owned name, so we also fetch champions whose canonical name differs from Gestal's.
  const { data: aliasRows } = await supabase.from('champion_aliases')
    .select('alias, champion_id').eq('game_id', 'raid_shadow_legends');
  const ownedNorm = new Set(ownedNames.map(norm));
  const aliasChampIds = [...new Set((aliasRows ?? [])
    .filter(a => ownedNorm.has(norm(a.alias))).map(a => a.champion_id))];

  // registry-exempt: candidate-widening PREFETCH (not an identity decision). Fetch by typeId (stable), by name,
  // AND by alias-resolved id, merge unique by id; the actual name→champions.id resolution is delegated to
  // buildUserChampions below. The name leg only pulls canonical-name/null-type_id champions into the pool.
  const byId = new Map();
