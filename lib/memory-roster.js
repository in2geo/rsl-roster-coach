...
import { STAT_KIND, GEAR_SET, SLOT, label } from './gear-labels.js';
...
export function readMemoryRoster({ run = true, exePath, outputDir, timeoutMs = 180_000 } = {}) {
  ...
  const heroes  = readJson(rosterFile);   // [{ heroId, typeId, baseTypeId, stars, level, ..., baseStats }]
  const rawGear = readJson(gearFile);     // [{ id, slotId, gearSetId, rarityId, rank, level, mainStatId, substats:[{statId,value}], equippedOnHeroId }]
  if (!Array.isArray(heroes) || !Array.isArray(rawGear)) return null;

  // Enrich each artifact with the STRING labels effectiveStats()/build-from-sync consume
  // (mainStat/stat/set/slot), alongside the numeric ids. gearSetId is the game ArtifactSetKindId;
  // mainStatId/statId are the Gestal statId scheme the reader already maps to.
  const artifacts = rawGear.map((a) => ({
    ...a,
    slot:     label(SLOT, a.slotId),
    set:      label(GEAR_SET, a.gearSetId),
    mainStat: label(STAT_KIND, a.mainStatId),
    substats: (a.substats ?? []).map((s) => ({ ...s, stat: label(STAT_KIND, s.statId) })),
  }));
  ...
  const champions = heroes.map((h) => ({
    ...
    baseStats:      h.baseStats ?? null,    // real base at level/stars → effectiveStats → sim total_stats
