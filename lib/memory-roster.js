...
// equipped gear + NAME (from the committed baseTypeId->name snapshot; see below). The
// name is load-bearing: the DB's champions.type_id is only ~25% populated, so without a
// name the fallback join drops ~half the roster (measured 36 vs 73 matched on DonaHilvi).
// Still NOT extracted from memory (degrade gracefully, as a hand-entered roster would):
//   • masteryIds  → []     (mastery_tier 'None', has_boss_mastery false)
//   • skills      → []     (is_booked false, book_fraction null → neutral)
//   • baseStats   → absent (effective_stats null → engine uses estimateStats)
// Extending the champion reader to close these is the follow-up to full Gestal parity.
...
export function readMemoryRoster({ run = true, exePath, outputDir, timeoutMs = 180_000 } = {}) {
  ...
  const champions = heroes.map((h) => ({
    ...
    masteryIds:     [],                     // degraded → mastery_tier 'None'
    skills:         [],                     // degraded → is_booked false / book_fraction null
