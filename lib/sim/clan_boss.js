...
import { flag, applyDebuff, dealDamage, defMitigation, effectiveDef } from './engine.js';
...
// Gathering Fury: ramps from turn 10, steeper from turn 20 (verbatim [T1]). ⚠ "turn" = the GLOBAL turn
// counter (state.turn = every champion action), NOT the boss's activations — confirmed by the turn-11
// capture: the boss's 11th activation dealt ~3.2× the turn-1 hit EVEN under Decrease ATK, only possible if
// ~60 global turns had passed (5 allies act between his turns). Rates calibrated with CB_BOSS_HIT.
const GF_RAMP_1 = Number(process.env.CB_GF1 ?? 0.05);   // per GLOBAL turn, turns 10-19
const GF_RAMP_2 = Number(process.env.CB_GF2 ?? 0.08);   // per GLOBAL turn, turns 20+
export function gatheringFury(t) {
  if (t < 10) return 1;
  if (t < 20) return 1 + GF_RAMP_1 * (t - 9);
  return 1 + GF_RAMP_1 * 10 + GF_RAMP_2 * (t - 19);
}
...
function makeDemonLord({ difficulty, aff, bossAtk }) {
  ...
  return {
    ...
    actEnemy(state, boss) {
      ...
      const hitAll = (hits) => {
        const perHit = (CB_BOSS_HIT[difficulty] ?? CB_BOSS_HIT.Hard) * gatheringFury(state.turn) * bossAtkFactor(boss);
      ...
      if (!flaggedOnce) {
        ...
        flag(state, 'BRACKETED: Crushing Force stun target = highest-MaxHP ally (placeholder targeting)');
      ...
      // ── A1 Crushing Force: single-target, unresistable 1-turn Stun + %MaxHP damage (DEF-independent),
      //    GF-escalated. Calibrated: turn-3 hit (gf=1) = 2,899 on Ezio → CB_CRUSH_PCT × Ezio MaxHP = 2,899.
      //    Targeting = highest-MaxHP ally (placeholder; real bias toward favourable/killable). ──
      const target = allies.reduce((m, a) => (a.maxHp > (m?.maxHp ?? -1) ? a : m), null);
      if (target) {
        applyDebuff(target, { type: 'Stun', turns: 1 });   // "cannot be resisted"
        if (doDamage) dealDamage(target, CB_CRUSH_PCT * gatheringFury(state.turn) * (target.maxHp ?? 0), 'direct', boss, state.allies);
      }
