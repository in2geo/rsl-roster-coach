...
import {
  dealDamage, scaleStat, critMult, defMitigation, effectiveDef, affinityFactor, dmgVariance,
...
function formulaBase(actor, F) {
  if (F.terms && F.terms.length) return F.terms.reduce((s, tm) => s + tm.coeff * scaleStat(actor, String(tm.stat).toLowerCase()), 0);
  return (F.multiplier ?? 0) * scaleStat(actor, String(F.scalingStat).toLowerCase());
}
...
function dealOneHit(state, actor, t, F, opponents, E) {
  ...
  let raw = formulaBase(actor, F) * critM * defM * affM * varM;
  ...
  const dd = dealDamage(t, raw, 'direct', actor, opponents);
...
export function installRecipeRun(state) {
  ...
  for (const a of state.allies) fireTriggers(state, a, 'round_start');   // round 1 = battle start
}
