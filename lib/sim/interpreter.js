...
function placeDebuffs(state, actor, act, intended, E, critActive = false) {
  ...
  for (const t of intended) {
    ...
    // Capture PRE-APPLY freeze state: a REFRESH of an already-frozen enemy is not a NEW "receives Freeze".
    const wasFrozen = ef.type === 'Freeze' && t.debuffs.some((x) => x.type === 'Freeze');
    ...
    // ON-FREEZE reaction (Hilvi Divine Mission): fires ONLY when an enemy NEWLY receives [Freeze]
    // (unfrozen → frozen), NOT on a refresh of an already-frozen enemy — Mike first-party 2026-08-03:
    // Ninja re-freezing already-frozen enemies produced NO TM drop across two observed turns. Any ally
    // carrying the enemy_frozen passive reacts on THAT enemy (HP Burn + steal buff + −TM). No-op for teams
    // without it → Dragon golden byte-identical. Gated on !wasFrozen so a refresh never re-triggers it.
    if (ef.type === 'Freeze' && t.side === 'enemy' && !wasFrozen && t.debuffs.some((x) => x.type === 'Freeze')) {
    ...
    for (const t of ctx.intended) {
      if (F.maxHpPct != null) {                       // DEF-independent %-of-target-MAX-HP nuke (capped on stage 21+/Hard)
        ...
        for (let h = 0; h < (F.hitCount ?? 1); h++) {
          ...
          if (F.onHitPlaceDebuff && ht && ht.hp > 0) placeDebuffs(state, actor, { effect: F.onHitPlaceDebuff }, [ht], E);
        }
...
function maybeEscalate(state, actor, recipe, results, E) {
  ...
  if (esc.round !== round) { esc.round = round; esc.hits = {}; }         // per-Round trigger window (multipliers persist)
