...
function formulaBase(actor, F) {
  ...
  if (F.terms && F.terms.length) return F.terms.reduce((s, tm) => s + tm.coeff * effectiveScaleStat(actor, String(tm.stat).toLowerCase()), 0);
  return (F.multiplier ?? 0) * effectiveScaleStat(actor, String(F.scalingStat).toLowerCase());
}
...
function dealOneHit(state, actor, t, F, opponents, E) {
  ...
  let raw = formulaBase(actor, F) * dynamicScaleFactor(state, F, actor, t) * critM * defM * affM * varM;
