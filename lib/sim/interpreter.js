...
const OP_HANDLERS = {
  ...
  FILL_TURN_METER(ctx, act)    { if (act.skipIfRevived && (ctx.revivedCount ?? 0) > 0) return; doFillTurnMeter(ctx.state, ctx.actor, act, ctx.E); },   // Hilvi A3: 25% TM fill fires ONLY when the same cast revived nobody
