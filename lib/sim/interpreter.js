...
import {
  dealDamage, effectiveScaleStat, critMult, effectiveCritRate, defMitigation, effectiveDef, affinityFactor, affinityMult, dmgVariance,
...
function dealOneHit(state, actor, t, F, opponents, E) {
  ...
  const fl = F.flags || {};
  const { raw, critM, affM } = computeRawHit(state, actor, t, F);
...
const OP_HANDLERS = {
  ACQUIRE_TARGETS(ctx, act) { ctx.intended = acquireTargets(ctx.state, ctx.actor, act, ctx.opponents, ctx.avoid); },
  DEAL_DAMAGE(ctx, act) {
