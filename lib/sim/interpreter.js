...
function dealOneHit(state, actor, t, F, opponents, E) {
  ...
  // [Enfeeble] on the ATTACKER forces a WEAK hit — "can only land weak hits" — overriding a neutral/strong
  // affinity. 0.70 is the same weak-hit multiplier affinityFactor uses (no new constant). This is the CONSUMER
  // for the Enfeeble Bambus A3 places on all wave enemies; it was placed-but-inert, so the mobs (incl. the
  // shield-piercing Faceless/Lua A3s that delete the revived Vergis) were hitting 30% too hard.
  const affM  = (actor.debuffs ?? []).some((d) => d.type === 'Enfeeble') ? 0.70 : (fl.affinity ? affinityFactor(actor.affinity, t.affinity) : 1);
  ...
  // A conditional ignore-DEF kicks in when the TARGET is under a named debuff (Pelops A2: ignore 50% DEF if
  // the target is under [HP Burn]). Take the larger of the static and conditional fractions. Reads data → NO constant.
  let ignoreDef = fl.ignore_def || 0;
  if (F.ignoreDefIfTargetUnder && (t.debuffs ?? []).some((d) => d.type === F.ignoreDefIfTargetUnder.debuff))
    ignoreDef = Math.max(ignoreDef, F.ignoreDefIfTargetUnder.ignore);
  ...
  // Pelops Master-of-Games: [HP Burn] on the attacker). Same helper the boss/mob paths use via the hook.
  if (incoming) fireDamageReactions(state, t, actor, dd.toHp);
  return {
...
const isUnresistable = (ef, t) =>
  ef.unresistable || (ef.unresistableIfTargetUnder && (t.debuffs ?? []).some((d) => d.type === ef.unresistableIfTargetUnder));
function placeDebuffs(state, actor, act, intended, E) {
  ...
  for (const t of intended) {
    ...
    if (ef.accuracy_check && !isUnresistable(ef, t)) {
...
export function fireTriggers(state, owner, eventName, ctx = {}) {
  ...
  for (const p of passives) for (const trig of p.triggers) {
    ...
    for (const act of trig.actions) {
      ...
      else if (act.op === 'WAKE_FROM_SLEEP') doWakeFromSleep(state, owner, E);
...
export function applyRecipe(state, actor, recipe) {
  ...
  for (const act of [...recipe.actions].sort((a, b) => a.seq - b.seq)) {
    if (act.op === 'ACQUIRE_TARGETS') {
      ...
      placeDebuffs(state, actor, act, act.target === 'self' ? [actor] : intended, E);
