...
import { RECIPES, FORMULAS, CONDITIONS, champKey } from './recipes.js';
...
export function recipeFor(actor, slot) {
  // champKey strips a fixture position suffix ("Lua#3" → "LUA") so wave mobs resolve to their champion recipe
  const first = champKey(actor.name);
...
function acquireTargets(state, actor, act, opponents, avoid) {
  ...
  if (act.target === 'single') { const s = chooseSingleTarget(opponents, avoid); if (s && actor.side === 'enemy') recordTargeting(state, opponents, s); return s ? [s] : []; }
...
function dealOneHit(state, actor, t, F, opponents, E) {
  ...
  if (actor.lifesteal > 0 && dealt > 0) {
    ...
    const heal = Math.min((actor.maxHp ?? 0) - actor.hp, actor.lifesteal * dealt * (1 - healReduction(actor)) * (state.lifestealFactor ?? 1));   // [Heal Reduction] on the attacker cuts its lifesteal
    if (heal > 0) { actor.hp += heal; actor.healed += heal; E({ kind: 'heal', subtype: 'Lifesteal', target: actor.name, fired: true, consumed: true, amount: heal }); }
  }
  ...
  if (F.critHealPct && critP > 0) {   // Lua A2: "each critical hit heals this Champion by 2.5% HP"
    ...
    if (heal > 0) { actor.hp += heal; actor.healed += heal; E({ kind: 'heal', subtype: 'crit-heal', target: actor.name, fired: true, consumed: true, amount: heal }); }
  }
...
function spongeOwner(state) {
  return state.allies.find((a) => a.alive && (a.debuffs ?? []).some((d) => d.type === 'Sleep')
    && Object.values(RECIPES).some((r) => r.type === 'passive' && r.sponge && champKey(r.champion) === champKey(a.name))) || null;
}
...
export function passiveImmunities(ownerName) {
  const first = champKey(ownerName);
  const out = [];
...
function isRecipeDriven(c) { return RECIPE_CHAMPS.has(champKey(c.name)); }
