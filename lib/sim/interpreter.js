...
function transferDebuff(dest, d) {
  ...
  if (cur) {
    cur.stacks = Math.min((cur.stacks ?? 1) + (d.stacks ?? 1), cap);
    cur.turnsLeft = Math.max(cur.turnsLeft ?? 0, d.turnsLeft ?? 0);
    if (d.value != null) cur.value = d.value;
    if (d.pct != null) cur.pct = d.pct;
    cur.maxStacks = cap;
  } else {
    dest.push({ type: d.type, value: d.value ?? null, pct: d.pct ?? null, turnsLeft: d.turnsLeft ?? 2, stacks: d.stacks ?? 1, maxStacks: cap });
  }
}
