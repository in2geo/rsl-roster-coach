...
function doRevive(state, actor, act, E) {
  ...
  return revived.length;   // callers (REVIVE handler) record this so a same-skill FILL_TURN_METER can gate on "no revive"
}
...
//   act.who: 'all_allies_except_self' (default; Fahrakin A3 Beatdown) | 'random_ally_except_self' (Pallas A1 —
//            one random ally joins) | 'all_allies'.  act.slot: the joining skill (default 'A1').
//   ⚠ FACTION GATE NOT ENFORCED: Pallas A1 restricts the joiner to an *Argonites* ally, but combatants carry no
//   faction, so a random living ally joins instead. On a real Argonites team every ally qualifies, so the only
//   miss is cross-faction over-inclusion of a minor damage add — documented on the recipe, not silently dropped.
function allyAttackJoiners(state, actor, who) {
  const side = actor.side === 'ally' ? state.allies : state.enemies;
  const own = side.filter((c) => c.alive && c !== actor);
  switch (who) {
    case 'all_allies': return side.filter((c) => c.alive);
    case 'random_ally_except_self': { if (!own.length) return []; const s = state?.rng?.target; return [own[s ? Math.floor(s() * own.length) : 0]]; }
...
function doAllyAttack(state, actor, act, intended, E) {
  ...
  const joiners = allyAttackJoiners(state, actor, act.who ?? 'all_allies_except_self');
  if (!joiners.length) { E({ kind: 'ally_attack', target: enemy.name, fired: true, consumed: false, note: 'no allies to join' }); return; }
...
function transferDebuff(dest, d) {
  ...
  if (cur) {
    ...
    cur.maxStacks = cap;
    // carry DoT ownership through the transfer (proportional to the stacks actually added under the cap)
...
function doWakeFromSleep(state, owner, E) {
  ...
  for (const d of moved) {
    ...
    if (d.type === 'Poison') d.sources = { [owner.name]: d.stacks ?? 1 };
...
const OP_HANDLERS = {
  ...
  REVIVE(ctx, act)             { ctx.revivedCount = (ctx.revivedCount ?? 0) + doRevive(ctx.state, ctx.actor, act, ctx.E); },
