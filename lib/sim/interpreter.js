...
// the owner's own. Pure redirect (reads no magnitude, introduces no constant beyond the card's 75%).
function maybeSponge(state, ally, type, E) {
  ...
  E({ kind: 'debuff', subtype: type, target: bambus.name, fired: true, consumed: true, note: 'sponged from ' + ally.name });
}
...
function doWakeFromSleep(state, owner, E) {
  ...
  for (const d of moved) { upsert(tgt.debuffs, { type: d.type, value: d.value, pct: d.pct, turns: d.turnsLeft }); E({ kind: 'debuff', subtype: d.type, target: tgt.name, fired: true, consumed: true, note: 'dumped from ' + owner.name }); }
}
...
function placeDebuffs(state, actor, act, intended, E) {
  ...
  for (const t of intended) {
    ...
    maybeSponge(state, t, ef.type, E);   // Sleeping Sage: a debuff on an ally transfers to an asleep Bambus
  }
}
