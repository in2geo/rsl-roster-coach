...
export function rollChance(stream, p) {
  if (!stream) { if (CHANCE_MODE === 'all') return true; if (CHANCE_MODE === 'none') return p >= 1; return p >= 1 || p > 0.5; }
  return stream() < p;
}
...
export function dealDamage(target, amount, kind = 'direct', attacker = null, team = null, ignoreShield = false) {
  ...
  if (kind === 'direct' && !ignoreShield) {   // ignoreShield: the hit bypasses [Shield]/[Magma Shield], straight to HP
...
export function simulate(state, content, { turnCap = 400, trace = false } = {}) {
  ...
  for (const phase of content.phases) {
    ...
    while (state.turn < turnCap) {
      ...
      if (!actor) break;
      state.turn += 1;
      state.onTurnStart?.(state, actor);   // recipe start-of-turn/round triggers (e.g. Ezio Perfect Veil renews)
...
function actChampion(state, c) {
  ...
  // RECIPE ROUTING: if a recipe executor is installed (the connected recipe run) and this skill has a recipe,
  // execute the RECIPE instead of the parsed-kit applySkill. Injected on state to avoid a circular import.
  if (state.recipeAct && state.recipeAct(state, c, skill)) return `${skill.slot} (recipe)`;
...
export function actEnemyMob(state, mob) {
  ...
  skill.cdLeft = skill.cooldown ?? 0;
  if (state.recipeAct && state.recipeAct(state, mob, skill)) return;   // recipe-driven mob (connected run)
