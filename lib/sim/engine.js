...
export function chooseSingleTarget(pool, avoid = []) {
  ...
  let base = targetable.length ? targetable : live;             // all veiled -> veil offers no cover
    ...
    for (const t of targets) {
      if (skill.maxHpPct != null) {
        ...
        dealDamage(t, scaleStat(actor, skill.coeffStat) * skill.coeff * critFactor(actor.critRate, actor.critDmg)
                    * defMitigation(effectiveDef(t)) * affinityFactor(actor.affinity, t.affinity), 'direct', actor, state.enemies);
...
function fireOnAttacked(state, defender, attacker) {
  for (const skill of defender.skills ?? []) {
    ...
    for (const d of skill.debuffs ?? []) {
      ...
      if (p != null && p <= 0.5) { flag(state, `on-attacked debuff likely resisted (${Math.round(p * 100)}%): ${defender.name} ${d.type}`); recordEffect(state, { source: `${defender.name} [P]`, kind: 'debuff', subtype: d.type, target: attacker.name, fired: true, consumed: false, note: `resisted ${Math.round(p * 100)}%` }); continue; }
...
export function actEnemyMob(state, mob) {
  ...
  for (const t of targets) {
    if (skill.maxHpPct != null) { dealMaxHpDamage(state, t, skill.maxHpPct, E, mob); }
      ...
      const before = t.hp + shieldPool(t);
      const dd = dealDamage(t, scaleStat(mob, skill.coeffStat) * skill.coeff * critFactor(mob.critRate, mob.critDmg) * defMitigation(effectiveDef(t)) * affinityFactor(mob.affinity, t.affinity), 'direct', mob, state.allies);
