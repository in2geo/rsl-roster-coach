  ...
  if (skill.hitsEnemies) {
    ...
    for (const t of targets) {
      if (skill.maxHpPct != null) {
        ...
        dealDamage(t, scaleStat(actor, skill.coeffStat) * skill.coeff * critMult(state, actor.critRate, actor.critDmg)
                    * defMitigation(effectiveDef(t)) * affinityFactor(actor.affinity, t.affinity) * dmgVariance(state), 'direct', actor, state.enemies);
    ...
    if (actor.bossMastery && targets[0] && WARMASTER_MAXHP > 0) {
      ...
      dealDamage(targets[0], wm, 'direct', actor, state.enemies);
...
export function actEnemyMob(state, mob) {
  ...
  for (const t of targets) {
    if (skill.maxHpPct != null) { dealMaxHpDamage(state, t, skill.maxHpPct, E, mob); }
      ...
      const before = t.hp + shieldPool(t);
      const dd = dealDamage(t, scaleStat(mob, skill.coeffStat) * skill.coeff * critMult(state, mob.critRate, mob.critDmg) * defMitigation(effectiveDef(t)) * affinityFactor(mob.affinity, t.affinity) * dmgVariance(state), 'direct', mob, state.allies);
      E({ kind: 'damage', target: t.name, fired: true, consumed: (before - (t.hp + shieldPool(t))) > 0, amount: before - (t.hp + shieldPool(t)) });
      if (dd.reflected > 0) E({ kind: 'reflect', subtype: 'Magma Shield', target: mob.name, fired: true, consumed: true, amount: dd.reflected });
    }
    for (const d of skill.debuffs ?? []) {
      if (t.immune?.includes(d.type)) { E({ kind: 'debuff', subtype: d.type, target: t.name, fired: true, consumed: false, note: 'immune' }); continue; }
      if (d.chance != null && !rollChance(state?.rng?.debuff, d.chance)) { E({ kind: 'debuff', subtype: d.type, target: t.name, fired: true, consumed: false, note: `missed placement (${Math.round(d.chance * 100)}%)` }); continue; }
      ...
      if (rollLand(state, p)) { applyDebuff(t, d); E({ kind: 'debuff', subtype: d.type, target: t.name, fired: true, consumed: t.debuffs.some(x => x.type === d.type) }); }
      else { flag(state, `mob debuff resisted (${Math.round(p * 100)}%): ${mob.name} ${d.type}`); E({ kind: 'debuff', subtype: d.type, target: t.name, fired: true, consumed: false, note: `resisted (${Math.round(p * 100)}% land)` }); }
    }
    fireOnAttacked(state, t, mob);   // the ATTACKED ally's passive punishes the mob (Pelops -> [HP Burn])
  }
}
