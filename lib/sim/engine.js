...
export function activatePoisons(state, t) {
  ...
  for (const d of poisons) {
    // CLAN BOSS: an ACTIVATED poison tick on the boss uses the SAME capped resolver as a natural tick.
    const dmg = (t.role === 'boss' && state.cbPoisonCaps) ? cbBossPoisonTick(state, t, d, sens)
              : (d.pct ?? 0.05) * (t.maxHp ?? 0) * (d.stacks ?? 1) * sens * bossPoisonFactor;
    if (t.role === 'boss' && state.cbPoisonCaps && state.cbLedger) state.cbLedger.poisonExact += dmg;
    t.hp -= dmg; dealt += dmg; t.taken += dmg; creditDot(state, t, 'Poison', dmg, d.sources);
  }
