...
export function applyWarmaster(state, actor, target, opponents, E) {
  ...
  const wmDefRetention = (tgt) => {
    ...
    // Same real DEF the direct-hit path uses: effectiveDef (boss.def × Decrease DEF) × Xeno −20% × gear ignore-DEF.
    const effDef = effectiveDef(tgt) * xenoDefMult(tgt) * (1 - (actor.ignoreDefStatic ?? 0));
