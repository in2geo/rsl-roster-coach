...
export function activatePoisons(state, t) {
  ...
  for (const d of poisons) { const dmg = (d.pct ?? 0.05) * (t.maxHp ?? 0) * (d.stacks ?? 1) * sens; t.hp -= dmg; dealt += dmg; creditDot(state, t, 'Poison', dmg, d.sources); }
...
export function dealDamage(target, amount, kind = 'direct', attacker = null, team = null, ignoreShield = false, ignoreBlockDamage = false) {
  ...
  if (reflected || reflectBuffDmg) {
    ...
    if (reflected && target.lifesteal > 0) target.hp = Math.min(target.maxHp ?? 0, target.hp + target.lifesteal * reflected * (1 - healReduction(target)));   // [Heal Reduction] cuts Magma-reflection lifesteal too
  }
...
export function upsert(list, e) {
  ...
  if (cur) {
    ...
    cur.stacks = Math.min(before + (e.stacking ? 1 : 0), e.maxStacks ?? 1);
    // DoT OWNERSHIP: track which champion placed each stack so damage is credited to the poisoner (creditDot).
    if (e.source && cur.stacks > before) { cur.sources ??= {}; cur.sources[e.source] = (cur.sources[e.source] ?? 0) + (cur.stacks - before); }
    return;
  }
  list.push({ type: e.type, value: e.value ?? null, pct: e.pct ?? null, turnsLeft: e.turns ?? 2, stacks: 1, placedTurn: CURRENT_TURN, sources: e.source ? { [e.source]: 1 } : undefined });
}
