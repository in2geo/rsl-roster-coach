 ...
 */
export function dealDamage(target, amount, kind = 'direct', attacker = null) {
    ...
    for (const t of targets) {
      if (skill.maxHpPct != null) {
        ...
        dealDamage(t, scaleStat(actor, skill.coeffStat) * skill.coeff * critFactor(actor.critRate, actor.critDmg)
                    * defMitigation(effectiveDef(t)) * affinityFactor(actor.affinity, t.affinity), 'direct', actor);
...
export function actEnemyMob(state, mob) {
  ...
  for (const t of targets) {
    if (skill.maxHpPct != null) { dealMaxHpDamage(state, t, skill.maxHpPct, E, mob); }
      ...
      const before = t.hp + shieldPool(t);
      const dd = dealDamage(t, scaleStat(mob, skill.coeffStat) * skill.coeff * critFactor(mob.critRate, mob.critDmg) * defMitigation(effectiveDef(t)) * affinityFactor(mob.affinity, t.affinity), 'direct', mob);
