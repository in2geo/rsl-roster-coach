...
export function simulate(state, content, { turnCap = 400, trace = false } = {}) {
  ...
  for (const phase of content.phases) {
    ...
    while (state.turn < turnCap) {
      ...
      // [Fear] / [True Fear] — turn-denial the hard-CC list above omits (glossary: [Fear] = 50% chance a skill
      // MISFIRES → the turn is forfeited; [True Fear] = guaranteed misfire, and additionally pushes the failed
      // skill onto cooldown). Placed-but-inert before this: a Feared enemy acted normally, so every Fear-locker
      // (Fabian's whole kit, Pharsalas, …) did nothing. True Fear always fires; Fear rolls 50% off the dedicated
      // 'fear' stream (seed=null → rollChance is p>0.5, false at exactly 0.5, so the deterministic golden never
      // mis-fires → byte-identical). Cooldowns/durations still tick on the skipped turn, as with Stun/Sleep.
      // (The True-Fear "push the failed skill to cooldown" surcharge is not modelled — flagged, low impact.)
      {
        ...
        if ((trueFear || fear) && (trueFear || rollChance(state?.rng?.fear, 0.50))) {
          ...
          recordEffect(state, { source: which, kind: 'cc', subtype: which, target: actor.name, fired: true, consumed: true, note: 'skill misfired (turn forfeited)' });
          if (trace) console.log(`  t${String(state.turn).padStart(3)} ${String(actor.name).slice(0, 14).padEnd(15)}${`-- ${which}: misfire --`.padEnd(22)}`);
          expireDurations(actor);
          tickCooldowns(actor);
          continue;
        }
