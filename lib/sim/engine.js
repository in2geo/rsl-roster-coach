 ...
 */
// Slots an authored recipe declares as `type: 'passive'`, keyed by recipeKey. A champion's skill can be
// mis-catalogued in the DB as an ACTIVE slot (no "[P]" suffix, e.g. Stag Knight's A3 "Lead the Pack", a
// no-cooldown reactive). pickSkill would then CAST it every turn (highest slot, no cooldown) and the champion
// would never attack. The recipe is the higher-fidelity source of truth for what is actually a passive, so we
// let it override the DB-derived isPassive. Only flips skills NOT already passive (DB "[P]" still wins on its
// own), so champions already handled via the "[P]" suffix (Tagoar/Michelangelo/Alice A4) are untouched.
const PASSIVE_RECIPE_SLOTS = (() => {
  const m = new Map();
  for (const r of Object.values(RECIPES)) {
    if (r.type !== 'passive') continue;
    const k = champKey(r.champion); if (!k) continue;
    if (!m.has(k)) m.set(k, new Set());
    m.get(k).add(String(r.slot ?? '').toUpperCase());
  }
  return m;
})();
...
export function makeCombatant(o) {
  ...
  const passiveSlots = PASSIVE_RECIPE_SLOTS.get(o.recipeKey ?? champKey(o.name ?? ''));
  const skills = passiveSlots
    ? (o.skills ?? []).map((s) => (!s.isPassive && passiveSlots.has(String(s.slot).toUpperCase()) ? { ...s, isPassive: true } : s))
    : (o.skills ?? []);
  return {
    name: o.name, side: o.side, role: o.role ?? 'champion',
    maxHp: o.maxHp ?? 0, hp: o.maxHp ?? 0,
    level: o.level ?? 60,                 // the unit's REAL in-game level (Dragon-20 enemies are genuinely L280).
                                          // NOTE: DEF-mitigation keys on the DEFENDER's level, not this — see defMitLevel().
    ...
    faction: o.faction ?? null,           // team-composition faction (Barbarians/Argonites/…) — gates faction-restricted ally-attacks (Pallas A1 "1 random Argonites ally")
    rarity: o.rarity ?? null,             // Rare/Epic/Legendary/Mythical — used by the Clan Boss HP-Burn cap (50k Rare / 75k Epic+)
    tags: o.tags ?? [],
    ...
    ignoreDefStatic: o.ignoreDefStatic ?? 0,   // STATIC gear Ignore DEF fraction (Savage/Lethal/Instinct/Cruel/Swift Parry 4-sets) — computeRawHit + applyWarmaster
    bookedSlots: o.bookedSlots ?? null,        // Set of MAXED skill slots (real per-account booked state) — gates per-skill book damage (F.bookDamage) in interpreter DEAL_DAMAGE
    skills,
...
// The LEVEL that drives an attacker's DEF-mitigation curve against a given defender.
//
// ⭐ FIRST-PARTY DERIVATION (Dragon st20, 2026-08-14): an enemy's DEF penetration keys on the DEFENDER's
// level, NOT the attacker's. Three independent st20 anchors agree, and the fixture path's old "attacker
// level" (enemies genuinely L280) contradicted all three:
//   1. Clean boss-AoE DEF-ratio (DEF 970 vs 1405): reality 1.26 = curve@L60 (1.23); @L280 it's 1.05.
//   2. Crossbowman A3 nuke on Mikey: reality 13,200 = ATK 7168 ×5.5 @L60 (12,810); @L280 = 23,061 (1.8× over).
//   3. Sanity: @L280 a basic attack (15,063) exceeds the nuke (13,200) — impossible; gone @L60.
// The enemy IS L280 (kept — its real ATK ladder, 11,947 at st20, is what makes it hurt at high stages);
// DEF mitigation is simply governed by the L60 champion's own level, so the stage's damage growth rides the
// ATK stat, not extra DEF penetration. SCOPE: applied to ENEMY→champ hits (the measured direction). Champ→enemy
// keeps the champion's own attacker level (unchanged) — the defender-enemy-level half is unmeasured; deferred.
export const defMitLevel = (attacker, defender) =>
  (attacker?.side === 'enemy' && defender ? (defender.level ?? 60) : (attacker?.level ?? 60));
...
// [Leech] heals the attacker 18% of Warmaster/GS bonus damage too (it IS "damage inflicted"). Default ON;
// SIM_WM_LEECH=off reverts (for A/Bs). See applyWarmaster's leechFrom().
export const WM_LEECH = process.env.SIM_WM_LEECH !== 'off';
...
// NOTE: streams are seeded BY INDEX, so APPEND new streams at the end — inserting mid-list reseeds every stream
// after it and drifts all snapshots. 'fear' (index 9) drives the [Fear] 50% misfire roll independently.
export const RNG_STREAMS = ['damage', 'crit', 'affinity', 'debuff', 'target', 'ai', 'mastery', 'gear', 'proc', 'fear'];
...
// Xenomorph's passive: while the target is under a Poison placed BY Xenomorph, its DEF is reduced 20%. This is a
// REAL conditional DEF reduction that stacks MULTIPLICATIVELY with Decrease DEF (0.40×0.80, NOT a max()), and it
// applies to ALL DEF-mitigated damage to that target — direct hits (computeRawHit) AND Warmaster (wmDefRetention).
// First-party: it is exactly what shifts the Dragon-boss Warmaster 41,103 → 47,542. `sources` carries the placer.
export const xenoDefMult = (t) => ((t?.debuffs ?? []).some((d) => d.type === 'Poison' && (d.stacks ?? 0) > 0
  && d.sources && Object.keys(d.sources).some((k) => /xeno/i.test(k))) ? 0.80 : 1);
...
export const CONSUMED_EFFECTS = new Set([
  ...
  'Unkillable',                           // checkDeaths (floors HP at 1 while up — cannot die)
  ...
  'Leech',                                // interpreter.dealOneHit (attacker heals 18% off a Leeched enemy)
  ...
  'Fear', 'True Fear',                     // turn loop: [Fear] 50% skill misfire, [True Fear] guaranteed misfire (turn forfeited)
]);
...
// A controlled unit TAKES its turn (it does not lose it) — DoTs tick, durations advance, it just can't cast.
// COOLDOWNS: Stun/Sleep/Petrification still drop by one on the skipped turn (Mike first-party 2026-08-03), but
// [Freeze] is the exception — its in-game tooltip states "Cooldowns are not refreshed while this debuff is
// active", so a frozen unit's cooldowns do NOT progress. This list = the CC types that FREEZE cooldowns.
export const CC_PRESERVES_COOLDOWNS = ['Freeze'];
...
// ── CLAN BOSS DoT resolvers (Infernal Resilience caps) — ONE source of truth so the NATURAL (tickDots)
//    and ACTIVATED (activatePoisons/activateHpBurns) paths can never diverge (a 5th path forgetting the cap
//    was reviewer issue #4). All gated by the caller on the CB caps being set → inert for other dungeons.
export function cbBossPoisonTick(state, t, d, sens) {
  const cap = state.cbPoisonCaps?.[(d.pct ?? 0.05) >= 0.05 ? '5' : '2.5'];   // by poison STRENGTH; 5% = 2× the 2.5% value
  return (cap != null ? cap : (d.pct ?? 0.05) * (t.maxHp ?? 0)) * (d.stacks ?? 1) * sens * (state.poisonDamageFactorVsBoss ?? 1);
}
// HP Burn on the boss is a FLAT tick by PLACER RARITY (Rare 50k / Epic+ 75k). d.sources = { placerName: stacks };
// resolve each placer to a combatant and read its rarity. Any Epic+ placer → 75k; ALL-Rare → 50k; unknown → 75k + flag.
export function cbBossBurnTick(state, d) {
  const names = d?.sources ? Object.keys(d.sources) : [];
  let sawRare = false, sawEpicPlus = false, sawUnknown = !names.length;
  for (const n of names) {
    const r = (state.allies?.find(a => a.name === n) ?? state.enemies?.find(e => e.name === n))?.rarity;
    if (r == null) sawUnknown = true; else if (/^rare$/i.test(r)) sawRare = true; else sawEpicPlus = true;
  }
  if (sawUnknown && !sawEpicPlus) flag(state, 'FLAGGED: HP Burn placer rarity unknown → 75k (Epic+) assumed');
  const rareOnly = sawRare && !sawEpicPlus && !sawUnknown;
  return rareOnly ? state.cbHpBurnCaps.rare : state.cbHpBurnCaps.epicPlus;
}
...
export function tickDots(state, c) {
  ...
  for (const d of [...c.debuffs]) {
    if (d.type === 'Poison') {
      ...
      // CLAN BOSS (Infernal Resilience): a Poison tick ON the boss is an ABSOLUTE per-tick CAP (by poison
      // STRENGTH 2.5%/5%), NOT %maxHP — the cap always binds. cbPoisonCaps is set ONLY by clan_boss content
      // (default null → Dragon/Spider byte-identical). The cap is the BASE tick; Poison Sensitivity (`sens`)
      // still multiplies it, and each stack ticks it (× stacks).
      let dmg;
      if (c.role === 'boss' && state.cbPoisonCaps) {
        dmg = cbBossPoisonTick(state, c, d, sens);         // shared capped resolver (natural + activated paths)
        if (state.cbLedger) state.cbLedger.poisonExact += dmg;   // CONFIRMED-DoT ledger (typed, reviewer #2)
      } else {
        dmg = (d.pct ?? 0.05) * c.maxHp * (d.stacks ?? 1) * sens * bossPoisonFactor;
      }
      ...
      // CLAN BOSS: HP Burn ON the boss is a FLAT tick by PLACER RARITY (Rare 50k / Epic+ 75k), not %maxHP —
      // via cbBossBurnTick (shared with the activated path). Gated on state.cbHpBurnCaps (clan_boss content
      // only → default null keeps other dungeons byte-identical). The "and all allies" splash stays %maxHP.
      const own = (c.role === 'boss' && state.cbHpBurnCaps) ? cbBossBurnTick(state, d) : 0.03 * c.maxHp;
      if (c.role === 'boss' && state.cbHpBurnCaps && state.cbLedger) state.cbLedger.hpBurnExact += own;
...
// STACKING-DoT OWNERSHIP. The canonical model is `d.stackList = [{turnsLeft, source}, ...]` — ONE entry per stack,
// binding each timer to its OWNER, so expiry drops the correct owner (the parallel stackTurns[] / sources{} arrays
// had no link between a timer and who placed it, so `sources` was never decremented on expiry → stale credit).
// `stacks` / `stackTurns` / `turnsLeft` / `sources` are DERIVED here for the consumers that still read them
// (tickDots / activatePoisons / creditDot / xenoDefMult / debuffSlots). INVARIANT (asserted in sim-selftest):
// sum(values(sources)) === stacks === stackList.length, after every placement / expiry / tick / activation.
// (pct stays a debuff-level field — every poison stack shares 5%; per-stack pct would need summing in tickDots.)
export function syncStackDerived(d) {
  const sl = d.stackList ?? [];
  d.stacks = sl.length;
  d.stackTurns = sl.map((s) => s.turnsLeft);
  d.turnsLeft = sl.length ? Math.max(...d.stackTurns) : 0;
  const src = {};
  for (const s of sl) if (s.source) src[s.source] = (src[s.source] ?? 0) + 1;
  d.sources = Object.keys(src).length ? src : undefined;
  return d;
}
// Migrate a legacy parallel-array poison (stackTurns[] + sources{}) to stackList, distributing owners across stacks.
// Only fires on an object created before this refactor (e.g. reused across trials) — fresh placements set stackList.
export function ensureStackList(d, fallbackTurns) {
  if (d.stackList) return d.stackList;
  const timers = d.stackTurns ?? Array(d.stacks ?? 1).fill(d.turnsLeft ?? fallbackTurns ?? 2);
  const owners = [];
  for (const [src, n] of Object.entries(d.sources ?? {})) for (let i = 0; i < Math.round(n); i++) owners.push(src);
  d.stackList = timers.map((t, i) => ({ turnsLeft: t, source: owners[i] }));
  return d.stackList;
}
// Force-ACTIVATE all [Poison] on a target: cash in each stack's FULL REMAINING DURATION now (every remaining
// tick, incl. [Poison Sensitivity]) and REMOVE the Poison (Mike-confirmed: poisons are removed when the skill
// lands). Debuff ACTIVATION (tag policy #12) = damage acceleration + slot relief — distinct from a natural tick,
// which deals ONE tick and leaves the Poison in place. CORRECTED 2026-08-16 from a one-tick burst: Mike's st20
// recording shows Ezio's explosion deal ~25% of Dragon maxHP (5 fresh 5% stacks × 2 turns × 2%/tick × 1.25
// [Poison Sensitivity] = 578,666, boss 100%→75%); the old one-tick model produced exactly half (12.5%/289,333),
// matched by the 200-seed measurement. Returns the damage dealt.
export function activatePoisons(state, t) {
  ...
  let dealt = 0;
  // The damage is CREDITED TO THE POISONER (each Poison's owner), not to Ezio — Raid's results screen attributes
  // it to whoever placed it (e.g. Bambus / Xenomorph). Per-stack duration → per-owner attribution stays exact.
  for (const d of poisons) {
    let dmg, weights;
    if (t.role === 'boss' && state.cbPoisonCaps) {
      // CLAN BOSS: the capped resolver (one Infernal-Resilience-capped tick × stacks) is calibrated on the
      // single-tick model — leave it; the full-duration burst below is the Dragon/generic activation.
      dmg = cbBossPoisonTick(state, t, d, sens); weights = d.sources;
      if (state.cbLedger) state.cbLedger.poisonExact += dmg;
    } else {
      // FULL REMAINING DURATION: sum every remaining tick of each stack (fresh 2-turn 5% stack = 2 ticks). All
      // stacks in a debuff share `pct`, so a stack's damage ∝ its turnsLeft → weight each owner by their Σ ticks.
      const perTick = (d.pct ?? 0.05) * (t.maxHp ?? 0) * sens * bossPoisonFactor;
      dmg = 0; weights = {};
      for (const s of ensureStackList(d)) {
        const ticks = Math.max(1, s.turnsLeft ?? 1);
        dmg += perTick * ticks;
        if (s.source) weights[s.source] = (weights[s.source] ?? 0) + ticks;
      }
    }
    t.hp -= dmg; dealt += dmg; t.taken += dmg; creditDot(state, t, 'Poison', dmg, weights);
  }
...
export function activateHpBurns(state, t) {
  ...
  for (const d of burns) {
    // CLAN BOSS: an ACTIVATED HP-Burn tick on the boss uses the SAME rarity-capped resolver as a natural tick.
    const own = (t.role === 'boss' && state.cbHpBurnCaps) ? cbBossBurnTick(state, d) : 0.03 * (t.maxHp ?? 0);
    if (t.role === 'boss' && state.cbHpBurnCaps && state.cbLedger) state.cbLedger.hpBurnExact += own;
    t.hp -= own; dealt += own; t.taken += own; creditDot(state, t, 'HP Burn', own, d.sources);
    for (const mate of alive(side)) {
      if (mate === t) continue;
...
let CURRENT_TURN = 0;
export function expireDurations(c) {
  ...
  for (const list of [c.buffs, c.debuffs]) {
    for (const e of [...list]) {
      ...
      if (e.stackList || e.stackTurns) {   // STACKING debuff (Poison): age each stack's own timer WITH its owner
        ensureStackList(e);
        e.stackList = e.stackList.map((s) => ({ ...s, turnsLeft: s.turnsLeft - 1 })).filter((s) => s.turnsLeft > 0);
        if (e.stackList.length === 0) { list.splice(list.indexOf(e), 1); continue; }   // all stacks (and owners) gone
        syncStackDerived(e);   // re-derive — sources now DROPS the expired stacks' owners (the stale-credit fix)
        continue;
      }
...
function checkDeaths(state) {
  // Loop so a chained kill (an [Infest] explosion that drops another unit ≤0) is resolved in the same pass.
  // For non-Infest content this runs exactly one effective pass then exits (second pass finds no new deaths) —
  // idempotent, golden-safe.
  let progressed = true;
  while (progressed) {
    progressed = false;
    for (const c of state.combatants) {
      if (c.alive && c.hp <= 0) {
        // [Unkillable]: cannot die while the buff is up — HP is floored at 1 instead of the death transition.
        // The buff does NOT absorb the hit (HP is already ≤0); it only blocks death, and expires on its own
        // duration, after which the next lethal hit kills normally. Mavara's A3 self-Unkillable is the designed
        // pair with her ally [Perfect Veil] (she is the untargetable-team's unkillable single-target soak).
        if (has(c, 'Unkillable')) { c.hp = 1; continue; }
        ...
        // ON-DEATH reactive hook: fire 'ally_death' for surviving same-side allies (Uugo Final Spite fires when
        // her LAST ally dies → she is the sole survivor). Injected by installRecipeRun; no-op on non-recipe runs.
        // Responses are self-buffs only (no attacks), so this cannot cascade. Golden-safe (no golden champ subscribes).
        state.onDeath?.(state, c);
        progressed = true;
        // [INFEST] DEATH-EXPLOSION (Xenomorph) — an Infested unit's death deals 50% of ITS max HP as PURE damage
        // (no DEF / affinity / crit) to all living units on its OWN side, chained (a resulting kill re-enters this
        // loop). Bosses & their spawned minions cap at 10% (glossary); ordinary wave mobs are NOT minions → full
        // 50%. Credited to the Infest placer. Mike first-party 2026-08-13 (HellHades-confirmed).
        const inf = c.debuffs?.find(d => d.type === 'Infest');
        if (inf) explodeInfest(state, c, inf);
      }
    }
  }
}
function explodeInfest(state, dead, inf) {
  const pct = (dead.role === 'boss' || dead.isMinion) ? 0.10 : 0.50;
  const dmg = (dead.maxHp ?? 0) * pct;
  if (!(dmg > 0)) return;
  const placer = inf.sources ? Object.keys(inf.sources)[0] : (inf.placedBy ?? 'Infest');
  for (const u of state.combatants) {
    if (u === dead || !u.alive || u.side !== dead.side) continue;
    u.hp -= dmg; u.taken = (u.taken ?? 0) + dmg;
    recordEffect(state, { source: placer, kind: 'damage', subtype: 'Infest', target: u.name, amount: dmg, fired: true, consumed: true, note: 'Infest death-explosion (pure)' });
  }
  state.log.push({ turn: state.turn, phase: state.phase, event: 'infest_explosion', who: dead.name, amount: Math.round(dmg) });
}
...
function killScore(state, attacker, c) {
  ...
  const dmg = Math.max(1, formulaBase(attacker, KILL_F, c) * defMitigation(effectiveDef(c), defMitLevel(attacker, c)) * AFF_REF(attacker.affinity, c.affinity));
  let hits = effHp / dmg;
...
export function simulate(state, content, { turnCap = 400, trace = false } = {}) {
  ...
  // CLAN BOSS absolute DoT caps + mastery gate + typed ledger + boss-turn clock (default null/false/0 → every
  // other dungeon byte-identical).
  state.cbPoisonCaps = content.cbPoisonCaps ?? null;                       // { '2.5': x, '5': y } per difficulty (tickDots/activatePoisons)
  state.cbHpBurnCaps = content.cbHpBurnCaps ?? null;                       // { rare, epicPlus } — HP-burn tick vs boss by placer rarity
  state.disableMasteryMaxHpVsBoss = content.disableMasteryMaxHpVsBoss ?? false;  // skip %maxHP mastery vs boss (applyWarmaster) — ONLY when no cap is supplied
  state.cbMasteryProcCap = content.cbMasteryProcCap ?? null;                     // CB: per-proc CAP on WM/GS %maxHP vs the boss (null → fall back to the disable gate)
  state.cbLedger = content.cbTrackDoT ? { poisonExact: 0, hpBurnExact: 0 } : null;   // typed CONFIRMED-DoT accumulator (reviewer #2)
  state.cbBossTurns = 0;                                                   // Demon Lord activation counter — clan_boss.actEnemy increments; content.endBattle reads (reviewer #1)
  ...
  for (const phase of content.phases) {
    ...
    while (state.turn < turnCap) {
      ...
      if (CC_SKIPS_TURN.some(t => actor.debuffs.some(d => d.type === t))) {
        ...
        recordEffect(state, { source: cc, kind: 'cc', subtype: cc, target: actor.name, fired: true, consumed: true, note: 'cast skipped (turn taken)' });
        if (trace) console.log(`  t${String(state.turn).padStart(3)} ${String(actor.name).slice(0,14).padEnd(15)}${'-- CC: cast skipped --'.padEnd(22)}`);
        // The controlled unit still TAKES its turn: DoTs already ticked at turn start (tickDots above), and its
        // durations advance here. Skill cooldowns drop by one for Stun/Sleep/Petrification (Mike first-party
        // 2026-08-03), EXCEPT under [Freeze] — its tooltip says cooldowns are not refreshed while frozen, so a
        // frozen unit keeps its cooldowns until it thaws (CC_PRESERVES_COOLDOWNS). It just misses the cast.
        expireDurations(actor);
        if (!actor.debuffs.some(d => CC_PRESERVES_COOLDOWNS.includes(d.type))) tickCooldowns(actor);
        continue;
      }

      // [Fear] / [True Fear] — RESTRICT the unit to its DEFAULT SKILL (A1); they do NOT skip the turn. GAME FACT
      // (Mike first-party 2026-08-13, HellHades-confirmed): a Feared unit still takes a full turn, but can only use
      // its default skill (A1) — [True Fear] always, [Fear] 50% of the time. This CORRECTS the earlier "misfire /
      // forfeit the turn" model (which zeroed a Feared mob's output; reality is it still fires its weak A1). The
      // restriction is consumed by pickSkill / pickEnemySkill via actor._forceDefaultSkill. Fear rolls 50% off the
      // dedicated 'fear' stream (seed=null → deterministic golden unaffected: no fear placed on golden teams).
      {
        const trueFear = actor.debuffs.some(d => d.type === 'True Fear');
        const fear = actor.debuffs.some(d => d.type === 'Fear');
        if ((trueFear || fear) && (trueFear || rollChance(state?.rng?.fear, 0.50))) {
          const which = trueFear ? 'True Fear' : 'Fear';
          actor._forceDefaultSkill = true;   // pickSkill/pickEnemySkill will return A1 for this turn's action
          recordEffect(state, { source: which, kind: 'cc', subtype: which, target: actor.name, fired: true, consumed: true, note: 'restricted to default skill (A1)' });
          if (trace) console.log(`  t${String(state.turn).padStart(3)} ${String(actor.name).slice(0, 14).padEnd(15)}${`-- ${which}: A1 only --`.padEnd(22)}`);
        }
      }
      ...
      // CLAN BOSS: the boss never dies — the key is TRUNCATED after his Nth activation (Gathering Fury wall,
      // reviewer #1). A wipe on the same turn already returned above; this is the survived-to-the-wall path.
      // `turnCap` here counts GLOBAL actions and is only a safety bound; the real clock is cbBossTurns.
      if (content.endBattle?.(state)) {
        phaseResults.push({ phase: phase.name, outcome: 'truncated', turns: state.turn - startTurn });
        return finish(state, true, `truncated after Demon Lord turn ${state.cbBossTurns}`, phaseResults);
      }
    }
...
function pickSkill(state, c) {
  // [Fear]/[True Fear] restrict to the DEFAULT skill (A1) for this turn — consume the flag and force A1.
  if (c._forceDefaultSkill) { c._forceDefaultSkill = false; return c.skills.find((s) => !s.isPassive && String(s.slot).toUpperCase() === 'A1') ?? c.skills.find((s) => !s.isPassive) ?? null; }
...
export function computeRawHit(state, actor, t, F) {
  ...
  ignoreDef = Math.min(1, ignoreDef + masteryIgnoreDef(state, actor) + (actor.ignoreDefStatic ?? 0));   // + Helmsmasher (mastery) + STATIC gear Ignore DEF (Savage/Lethal/Instinct/Cruel/Swift Parry 4-sets), applied to DEF before the curve
  const defForHit = effectiveDef(t) * xenoDefMult(t) * (1 - ignoreDef);   // xenoDefMult: Xenomorph's conditional −20% boss DEF (his passive), multiplicative with Decrease DEF
  const defM = fl.def_mitigation ? defMitigation(defForHit, defMitLevel(actor, t)) : 1;
  ...
  // [Freeze] on the TARGET: it "only receives 75% of incoming damage" (in-game Freeze tooltip, Mike first-party
  // 2026-08-03) — a flat 25% cut applied here so it covers BOTH directions (ally→enemy and enemy→ally); the
  // enemy→ally-only incomingDamage() path can't. DoT ticks (tickDots) do not run through computeRawHit, so HP
  // Burn on a frozen mob still lands full, matching the replay. Freeze-specific (Stun/Sleep tooltips differ).
  const frzM = (t.debuffs ?? []).some((d) => d.type === 'Freeze') ? 0.75 : 1;
  let raw = formulaBase(actor, F, t) * dynamicScaleFactor(state, F, actor, t) * critM * defM * affM * varM * frzM * (actor.dmgMult ?? 1) * masteryDamageMult(state, actor, t, F);
...
export function applyWarmaster(state, actor, target, opponents, E) {
  if (!target) return;
  // [Leech] on the target: Warmaster/Giant Slayer bonus damage IS "damage inflicted" by the attack, so it leeches
  // too (18%, Heal-Reduction gated) — mirrors interpreter.dealOneHit. Without this the team's Warmaster (its
  // BIGGEST leechable chunk) heals nobody: on DonaHilvi st20 the Warmaster attackers self-sustained 60–88% short
  // while Ezio — no boss mastery, no Warmaster — matched reality. Poison (a DoT tick, not an attack) still does
  // NOT leech (and would over-heal 7×), matching the interpreter's direct-attack-only rule.
  const leechFrom = (amt) => {
    if (WM_LEECH && amt > 0 && (target.debuffs ?? []).some((d) => d.type === 'Leech')) {
      const heal = 0.18 * amt * (1 - healReduction(actor));
      if (heal > 0) { actor.hp = Math.min(actor.maxHp ?? 0, actor.hp + heal); actor.healed = (actor.healed ?? 0) + heal; E({ kind: 'heal', subtype: 'Leech', target: actor.name, fired: true, consumed: true, amount: heal }); }
    }
  };
  // DEF-MITIGATION of Warmaster/GS bonus damage (Mike first-party, Dragon st20). The raw 4%×maxHP is reduced by
  // the boss's DEF retention through the SAME curve as direct hits — reproduces 41,103 (Dec-DEF) / 47,542 (Dec-DEF
  // + Xeno Poison) EXACTLY. Uses the boss's FULL displayed DEF (displayDef, = 3,982 @ st20), NOT the
  // BOSS_DEF_FACTOR-scaled boss.def direct hits use. Folds in Decrease DEF (statFactor) and Xenomorph's CONDITIONAL
  // −20% (only while the target is under a Poison whose `sources` include Xenomorph — his passive). Non-boss
  // targets (waves) return 1 (no anchor; leaves wave-phase Warmaster unchanged). Level 60 → −effDef/1500.
  const wmDefRetention = (tgt) => {
    if (tgt.role !== 'boss') return 1;
    // Real DEF: effectiveDef (boss.def × Decrease DEF) × Xeno −20% passive. NOTE: static gear Ignore-DEF is
    // deliberately NOT applied here — the captures prove Warmaster is DEF-MITIGATED, but do NOT prove that
    // Savage/Lethal/Cruel Ignore-DEF applies to the Warmaster PROC (it's a mastery bonus, not the skill hit).
    // Kept direct-hit-only until a Warmaster-with-Ignore-DEF anchor exists (untestable on this 0%-IgnoreDEF team).
    const effDef = effectiveDef(tgt) * xenoDefMult(tgt);
    return defMitigation(effDef, 60);
  };
  // CLAN BOSS: Warmaster / Giant Slayer ARE capped on the Demon Lord, but the per-proc capped VALUE is
  // unknown → skip the %maxHP bonus vs the boss (rather than credit the huge uncapped %maxHP). Gated on
  // state.disableMasteryMaxHpVsBoss (clan_boss content only). The content flags this once; no per-proc spam.
  // CLAN BOSS: WM/GS %maxHP IS capped per proc on the Demon Lord (uncapped 4%×~194M HP is absurd). When the
  // CB content supplies state.cbMasteryProcCap, cap the per-proc value and APPLY it; only when no cap is set
  // does the legacy disable skip it entirely.
  if (state?.disableMasteryMaxHpVsBoss && target.role === 'boss' && state?.cbMasteryProcCap == null) return;
  ...
  if (bb) {
    ...
    let perProc = coeff * (target.maxHp ?? 0);
    if (target.role === 'boss' && state?.cbMasteryProcCap != null) perProc = Math.min(perProc, state.cbMasteryProcCap);   // CB per-proc cap (Demon Lord)
    perProc *= wmDefRetention(target);   // Dragon boss: 4%×maxHP × DEF retention (1 vs CB/waves) — see wmDefRetention
    const stream = state?.rng?.mastery;
    const dmg = stream ? (stream() < bb.chance ? perProc : 0) : bb.chance * perProc;
    if (dmg > 0) { dealDamage(target, dmg, 'direct', actor, opponents); leechFrom(dmg); E({ kind: 'damage', subtype: bb.name, target: target.name, fired: true, consumed: true, amount: dmg, rolled: !!stream }); }
  ...
  const lCap = (target.role === 'boss') ? (state?.cbMasteryProcCap ?? null) : null;   // CB per-proc cap only (Demon Lord)
  const ret = wmDefRetention(target);   // Dragon boss: DEF retention on the raw %maxHP (1 vs CB/waves)
  const stream = state?.rng?.mastery;
  const rawPer = (lCap != null ? Math.min(WARMASTER_RAW * (target.maxHp ?? 0), lCap) : WARMASTER_RAW * (target.maxHp ?? 0)) * ret;
  const ev = (lCap != null ? Math.min(WARMASTER_MAXHP * (target.maxHp ?? 0), WARMASTER_PROC * lCap) : WARMASTER_MAXHP * (target.maxHp ?? 0)) * ret;
  const wm = stream ? (stream() < WARMASTER_PROC ? rawPer : 0) : ev;
  if (wm > 0) { dealDamage(target, wm, 'direct', actor, opponents); leechFrom(wm); E({ kind: 'damage', subtype: 'Warmaster', target: target.name, fired: true, consumed: true, amount: wm, rolled: !!stream }); }
...
export function upsert(list, e) {
  ...
  if (cur) {
    ...
    if (e.stacking) {
      // STACKING debuff (Poison): each application is an INDEPENDENT stack with its OWN duration AND its OWN owner
      // ({turnsLeft, source} in stackList). A new application NEVER refreshes existing stacks (the RSL rule) — they
      // age out one at a time. A rejected stack at the cap adds NEITHER a timer NOR ownership. (Was: parallel
      // stackTurns[]/sources{} with no timer↔owner link → sources never decremented on expiry = stale credit.)
      ensureStackList(cur, e.turns);
      const cap = e.maxStacks ?? 1;
      if (cur.stackList.length < cap) {
        cur.stackList.push({ turnsLeft: e.turns ?? 2, source: e.source });   // new stack: own timer + own owner
  ...
  const nd = { type: e.type, value: e.value ?? null, pct: e.pct ?? null, turnsLeft: e.turns ?? 2, stacks: 1,
    placedTurn: CURRENT_TURN, ...(e.placedBy ? { placedBy: e.placedBy } : {}) };
  if (e.stacking) { nd.stackList = [{ turnsLeft: e.turns ?? 2, source: e.source }]; syncStackDerived(nd); }   // per-stack ownership
  else nd.sources = e.source ? { [e.source]: 1 } : undefined;   // non-stacking: single owner, no per-stack timers
  list.push(nd);
}
