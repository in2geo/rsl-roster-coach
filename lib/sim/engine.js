...
export function activatePoisons(state, t) {
  ...
  let dealt = 0;
  for (const d of poisons) { const dmg = (d.pct ?? 0.05) * (t.maxHp ?? 0) * (d.stacks ?? 1) * sens; t.hp -= dmg; dealt += dmg; }
...
export function upsert(list, e) {
  ...
  list.push({ type: e.type, value: e.value ?? null, pct: e.pct ?? null, turnsLeft: e.turns ?? 2, stacks: 1, placedTurn: CURRENT_TURN });
}
