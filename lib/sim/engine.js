...
export function actEnemyMob(state, mob) {
  ...
  if (!skill) return;
  skill.cdLeft = skill.cooldown ?? 0;
  const targets = skill.aoe ? allies : [allies.reduce((a, b) => (a.hp / a.maxHp <= b.hp / b.maxHp ? a : b))];
  for (const t of targets) {
    if (skill.coeff == null) flag(state, `MISSING damage_multiplier (mob): ${mob.name} ${skill.slot}`);
    else dealDamage(t, mob.atk * skill.coeff * critFactor(mob.critRate, mob.critDmg) * defMitigation(t.def) * affinityFactor(mob.affinity, t.affinity), 'direct');
    for (const d of skill.debuffs ?? []) {
      if (t.immune?.includes(d.type)) continue;
      const p = landChance(mob.acc, t.res);
      if (p == null) { flag(state, `UNKNOWN land chance (mob): ${mob.name} ${d.type}`); continue; }
      if (p >= 1 || p > 0.5) applyDebuff(t, d);
      else flag(state, `mob debuff likely resisted (${Math.round(p * 100)}%): ${mob.name} ${d.type}`);
    }
  }
}
