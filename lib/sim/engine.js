...
import { RECIPES, champKey } from './recipes.js';   // read-only: incomingDamage() scans champions' `modifiers` (recipes.js imports nothing → no cycle)
...
export function makeCombatant(o) {
  return {
    ...
    taken: 0, healed: 0,                  // SURVIVAL-oracle accounting: cumulative HP damage taken / HP healed (records only, never read by battle logic → golden-safe). Mirrors the in-game results screen's blue (taken) / green (healed) bars.
    buffs: [], debuffs: [],
...
export function tickDots(state, c) {
  ...
  for (const d of [...c.debuffs]) {
    if (d.type === 'Poison') {
      ...
      // ATTRIBUTION (mirror of Poison): credit the FULL HP Burn tick (own + splash) to the champion that
      // PLACED it, via `sources`. On Spider this IS the Pelops engine — his HP Burn on the Spiderlings
      // splashes onto Skavag every tick, and all of it is HIS damage. Falls back to 'HP Burn' if unowned.
      // (No boss-Poison reduction here — Healing Assured cuts POISON on Skavag, not HP Burn.)
      creditDot(state, c, 'HP Burn', own + splash, d.sources);
    }
  }
  return dealt;
}
...
export function activatePoisons(state, t) {
  ...
  for (const d of poisons) { const dmg = (d.pct ?? 0.05) * (t.maxHp ?? 0) * (d.stacks ?? 1) * sens; t.hp -= dmg; dealt += dmg; t.taken += dmg; creditDot(state, t, 'Poison', dmg, d.sources); }
...
function tickHots(state, c) {
  for (const b of c.buffs) {
    ...
    const before = c.hp;
    c.hp = Math.min(c.maxHp, c.hp + ((b.value ?? 15) / 100) * (c.maxHp ?? 0) * (1 - healReduction(c)));   // [Heal Reduction] cuts the [Continuous Heal] tick
    c.healed += c.hp - before;
...
function expireDurations(c) {
  for (const list of [c.buffs, c.debuffs]) {
    for (const e of [...list]) {
      ...
      // A reactive [Petrification] lands on the ATTACKING mob mid-turn (Pelops's on-attacked passive). Raid
      // counts its 1-turn duration in the mob's OWN turns, so it must survive the placement turn's end-expiration
      // and first tick (→ skip) at the mob's NEXT turn. Without this guard the fresh 1-turn petrif was ticked
      // 1→0 the same turn and removed before it could skip anything — it landed but caused 0 skips (the wave-2
      // survival bug). Scoped to [Petrification] only: the turn-skipping CC whose whole value is the skip, and
      // the one verified against reality. Other effects keep their existing decrement timing (no model-wide shift).
      if (e.type === 'Petrification' && e.placedTurn === CURRENT_TURN) continue;
...
export function dealDamage(target, amount, kind = 'direct', attacker = null, team = null, ignoreShield = false, ignoreBlockDamage = false) {
  ...
  target.hp -= left; target.taken += left;   // SURVIVAL-oracle: cumulative HP damage taken (post-shield)
  ...
  if (reflected || reflectBuffDmg) {
    ...
    if (reflected && target.lifesteal > 0) { const before = target.hp; target.hp = Math.min(target.maxHp ?? 0, target.hp + target.lifesteal * reflected * (1 - healReduction(target))); target.healed += target.hp - before; }   // [Heal Reduction] cuts Magma-reflection lifesteal too
  }
...
export function incomingDamage(state, target, amount) {
  ...
  for (const owner of side.filter((c) => c.alive)) {
    const first = champKey(owner.name);
...
export function computeRawHit(state, actor, t, F) {
  const fl = F.flags || {};
  const cr = fl.crit ? rollCrit(state, effectiveCritRate(actor), actor.critDmg) : { crit: false, mult: 1, ev: false };
  const critM = cr.mult;
...
export function upsert(list, e) {
  ...
  if (cur) {
    ...
    if (/Shield/.test(e.type)) { cur.turnsLeft = Math.max(cur.turnsLeft, e.turns ?? 2); cur.value = e.value ?? cur.value; return; }   // 'override', not additive
    if (e.stacking) {
