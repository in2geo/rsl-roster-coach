...
export function simulate(state, content, { turnCap = 400, trace = false } = {}) {
  ...
  state.disableMasteryMaxHpVsBoss = content.disableMasteryMaxHpVsBoss ?? false;  // skip %maxHP mastery vs boss (applyWarmaster)
...
export function applyWarmaster(state, actor, target, opponents, E) {
  ...
  if (state?.disableMasteryMaxHpVsBoss && target.role === 'boss') return;
