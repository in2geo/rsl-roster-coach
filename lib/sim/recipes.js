...
export const RECIPES = {
  ...
  'GLORIOUS-A1': {
    ...
    // "…with 1 random ARGONITES ally (uses default skill)": the ALLY_ATTACK op exists (Fahrakin A3 uses it), but
    // the Argonites FACTION gate is not enforceable — combatants carry affinity, not faction. Granting the join to
    // any random ally over-credits NON-Argonites teams (which have no eligible joiner, so no join fires in reality)
    // → measured −1.1pp on Spider (one extra false-clear) in the sim-suite A/B, 2026-08-12. DEFERRED until faction
    // is plumbed onto combatants (dragon-fixture SEL + makeCombatant); then who:'random_faction_ally', faction:'Argonites'.
    deferred: ['a random Argonites ally joins the attack with its default skill — ALLY_ATTACK op built, but the Argonites faction gate needs faction data on combatants (not yet plumbed); modelling it unconditionally regressed Spider −1.1pp'],
  },
  'GLORIOUS-A2': {
