...
export const RECIPES = {
  ...
  'BAMBUS-A2': {
    ...
    // ⚠ DEFERRED — MAGNITUDE BASE UNVERIFIED: "increases the value of any [Shield] buffs on all allies by 3% for
    // each enemy buff that had its duration decreased." 3% OF WHAT (the shield's current value vs Bambus MAX HP)
    // is ambiguous in the skill text; per IMPLEMENT-DON'T-FIT we do not guess the base. Needs Mike/in-game
    // confirmation. Cross-action coupling (count from REDUCE_EFFECT_DURATION) is trivial once the base is known.
    deferred: ['+3% to ally [Shield] value per enemy buff decreased — base (%shield vs %MaxHP) UNVERIFIED (II-C)'],
  },
