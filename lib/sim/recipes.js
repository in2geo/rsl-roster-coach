...
export const FORMULAS = {
  ...
  F_XENO_A2: { scalingStat: 'ATK', multiplier: 5.9, terms: null, flags: { ...DMG_FLAGS }, hitCount: 1 },   // Infestation (DB 5.9 ATK)
  ...
  F_MICHELANGELO_A1: { scalingStat: 'ATK', multiplier: 2, terms: null, flags: { ...DMG_FLAGS }, hitCount: 2, bookDamage: 0.20 },   // "Attacks 1 enemy 2 times". bookDamage = four sequential Damage +5% upgrades → ×1.20 at A1 level 5, SEPARATE from the 2.0× coefficient; applied ONLY when this champion's A1 is booked (interpreter DEAL_DAMAGE gates on actor.bookedSlots). Reproduces the captured 2,393 normal-hit anchor.
...
export const RECIPES = {
  ...
  // A2 — Infestation (5.9 ATK, cd4). AUTHORED 2026-08-13 (was falling through to kitToRecipe, which placed NO
  // Infest and NO True Fear). Places [Stun] + [INFEST] on the target and [True Fear] on all OTHER enemies (the
  // stunned primary target getting True Fear too is inert). Debuffs are unresistable under Xeno's [Perfect Veil]
  // (he holds it via A1/A3) → modelled as guaranteed (accuracy_check off). [Infest] itself is the death-bomb:
  // when an Infested enemy dies, engine.explodeInfest deals 50%·its-maxHP PURE AoE to the other enemies (chains).
  // On DonaHilvi Dragon st20 this is the wave-clear ENGINE (poison kills → Infest chain-detonation). Mike video 2026-08-13.
  'XENOMORPH-A2': {
    champion: 'Xenomorph', slot: 'A2', name: 'Infestation', type: 'active', cooldown: 4,
    actions: [
      { seq: 10, phase: 'skill_start', op: 'ACQUIRE_TARGETS', repeat: 'once', target: 'single' },
      { seq: 20, phase: 'per_target', op: 'DEAL_DAMAGE', target: 'current_target', repeat: 'once', formulaId: 'F_XENO_A2' },
      { seq: 30, phase: 'after_hit', op: 'PLACE_DEBUFF', target: 'intended_set', repeat: 'per_target', effect: { type: 'Stun', duration: 2, chance: 1, accuracy_check: false } },
      { seq: 32, phase: 'after_hit', op: 'PLACE_DEBUFF', target: 'intended_set', repeat: 'per_target', effect: { type: 'Infest', duration: 2, chance: 1, accuracy_check: false } },
      { seq: 40, phase: 'after_skill', op: 'ACQUIRE_TARGETS', repeat: 'once', target: 'all_enemies' },
      { seq: 42, phase: 'after_skill', op: 'PLACE_DEBUFF', target: 'intended_set', repeat: 'per_target', effect: { type: 'True Fear', duration: 1, chance: 1, accuracy_check: false } },
    ],
    covers: ['Stun + Infest on target (Infest = 50%·maxHP pure death-explosion, engine.explodeInfest)', 'True Fear on all enemies (restrict to A1)'],
    deferred: ['revive-on-Infest-death passive (Xeno self-revive when an Infested enemy dies) — separate passive, not wired'],
  },
  ...
  'NINJA-A3': {
    ...
    actions: [
      ...
      // "Will also decrease the cooldown of the Hailburn skill by 1 turn." Self-target, own A2 only — more
      // frequent Hailburn = more HP-Burn detonations, Ninja's boss-damage engine (DECREASE_COOLDOWN self path).
      { seq: 40, phase: 'after_skill', op: 'DECREASE_COOLDOWN', target: 'self', repeat: 'once', effect: { self: true, slot: 'A2', turns: 1 } },
    ],
    covers: ['decrease the cooldown of the Hailburn (own A2) skill by 1 turn (DECREASE_COOLDOWN self)'],
    deferred: [],
  },
