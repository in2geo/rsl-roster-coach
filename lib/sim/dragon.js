...
import { flag, applyDebuff, landDebuff, has, defMitigation, defMitLevel, effectiveDef, dealDamage, incomingDamage, affinityMult, recordEffect } from './engine.js';
...
// Champ→boss DEF: the boss's EFFECTIVE DEF vs champ direct hits is ~0.43× its DISPLAYED DEF (~3,992), MEASURED
// from three first-party Ezio hits on CONFIRMED gear (2026-08-14). The clean anchor — st19 A1 = 4,164 CRIT with
// NO buffs and NO Dec-DEF (full DEF), in-battle ATK ~1,698, C.DMG 109% → retention 0.42 → base effective DEF
// ~1,725. It then predicts the st19 Dec-DEF hit (6,864) to <1%, and matches the st20 no-buff hit (3,103 → 1,633).
// The displayed DEF is flat across stages (3982–4102), so ONE factor covers 1–25 (no stage curve). Same
// displayed≫effective gap cb.mjs found on CB (effective ≈0) — a PER-BOSS effective DEF, NOT a global formula
// constant. Without this the sim under-credits champ→boss direct ~1.55× (Dec-DEF) to ~2.0× (full DEF); it is the
// counterweight to poisonDamageFactorVsBoss (0.40) — the two must move together (MODEL_AS_REIMPLEMENTATION coupling).
export const BOSS_DEF_FACTOR = Number(process.env.SIM_DRAGON_BOSS_DEF_FACTOR ?? 0.43);
...
export function makeDragonContent({ stageNumber, purpleBarHp = null, waves = null, difficulty = 'Normal', boss }) {
  ...
  // Apply the measured champ→boss effective-DEF factor once (see BOSS_DEF_FACTOR). Only the boss's OWN def is
  // touched — it is consumed solely when a champ hits the boss (effectiveDef(boss)); the boss's attacks scale off
  // the TARGET's def, so this cannot leak into the boss→ally direction. Guarded so a reused object isn't scaled twice.
  if (boss && !boss._dragonDefScaled) { boss.def = Math.round((boss.def ?? 0) * BOSS_DEF_FACTOR); boss._dragonDefScaled = true; }
