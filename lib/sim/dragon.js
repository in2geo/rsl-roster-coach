...
import { flag, applyDebuff, has, defMitigation, effectiveDef, dealDamage, incomingDamage } from './engine.js';
...
function makeHellrazor({ stageNumber, purpleBarHp = null }) {
  ...
  return {
    ...
    actEnemy(state, boss) {
      ...
      const strike = (a) => { const dd = dealDamage(a, incomingDamage(state, a, bossHit(boss, a)), 'direct', boss, state.allies); state.onDamageTaken?.(state, a, boss, dd.toHp); };
      ...
      if (scorchArmed) {
        ...
        if (purpleBarHp == null && mode !== 'always') {
          ...
          for (const a of allies) { strike(a); applyDebuff(a, { type: 'Stun', turns: 1 }); }
...
function bossHit(boss, target) {
  ...
  return boss.atk * bossAtkMod * defMitigation(effectiveDef(target), boss.level);   // level-scaled DEF curve; a DEF-shred on the ally boosts his hit
}
