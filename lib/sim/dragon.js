...
import { flag, applyDebuff, has, defMitigation, dealDamage } from './engine.js';
 ...
 */
export function makeDragonContent({ stageNumber, purpleBarHp = null, waves = null, boss }) {
    ...
    actEnemy(state, boss) {
      ...
      if (scorchArmed) {
        ...
        if (purpleBarHp == null && mode !== 'always') {
          ...
          for (const a of allies) { dealDamage(a, bossHit(boss, a), 'direct'); applyDebuff(a, { type: 'Stun', turns: 1 }); }
          ...
          for (const a of allies) {
            dealDamage(a, bossHit(boss, a), 'direct');
...
function bossHit(boss, target) {
  ...
  return boss.atk * bossAtkMod * defMitigation(target.def);
}
