...
import { flag, makeCombatant, applyDebuff, dealDamage, computeRawHit, decreaseTurnMeter, chooseAllyTarget, rollChance } from './engine.js';
...
export function makeSpiderContent({ stageNumber, boss, spawnTemplate, difficulty = 'Normal' }) {
  ...
  function actSpider(state, unit) {
    ...
    if (consumeEligible && spiders.length > 0) {
      ...
      boss.atk = Math.round(boss.atk * (1 + 0.10 * n));                    // PERMANENT ATK gain
  ...
  function spiderlingAct(state, sp) {
    const target = chooseAllyTarget(state.allies);                         // single-target
    if (!target) return;
    ...
    for (let i = 0; i < 2; i++) applyDebuff(target, { type: 'Poison', pct: 0.05, turns: 2, stacking: true, maxStacks: 10 });   // TWO 5% MaxHP Poisons per hit
  }
