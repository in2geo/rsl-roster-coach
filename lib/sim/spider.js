...
// DAMAGE MULTIPLIERS below are datamined (community, NOT Plarium-published — flagged; see the design doc).
// The COOLDOWN (STUPEFYING_SILK_CD) is NOT datamined — it is factual game data (Silk cd 4, hand-read from
// AyumiLove's Spider guide; on record in SPIDER_REVIEW.md:17). Don't tag the cd as a guess alongside the coeffs.
const VENOM_SPRAY_COEFF = 2.5;        // AoE ATK multiplier (datamined), ×1.15 vs a Poisoned target
const STUPEFYING_SILK_COEFF = 3.0;    // AoE ATK multiplier (datamined); the skill also does 70% −30% TM, Sleep-on-TM-empty
const STUPEFYING_SILK_CD = 4;         // KNOWN cd 4 → Silk every 4th boss turn (this cast + 3 Venoms), matching the engine's champion-cooldown convention (engine.js tickCooldowns + cdLeft=cooldown). The old code fired it every 5th turn — a deviation from the known cd, corrected 2026-08-07.
...
export function makeSpiderContent({ stageNumber, boss, spawnTemplate, difficulty = 'Normal' }) {
  ...
  function actSpider(state, unit) {
    ...
    if (silkCd <= 0) {
      silkCd = STUPEFYING_SILK_CD - 1;   // −1: the guard checks BEFORE the else-branch decrement, so reset to cd−1 → 3 Venoms between Silks (period 4, not 5)
  ...
  function spiderlingAct(state, sp) {
    ...
    let placed = 0;
    // count a placement only when landDebuff ACTUALLY lands it — it already gates on target.alive + immunity +
    // ACC/RES resist and RETURNS the result. Incrementing on the 0.37 roll alone fired Bambus's sponge on
    // resisted/blocked/on-a-corpse "placements" that don't exist (reviewer findings 4 & 5).
