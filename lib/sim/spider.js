...
import { flag, makeCombatant, applyDebuff, landDebuff, dealDamage, computeRawHit, decreaseTurnMeter, chooseAllyTarget, rollChance, recordEffect } from './engine.js';
...
import { champKey } from './recipes.js';   // records-only: identify Pelops in the Taunt-trace telemetry
...
// keep her fair share.
const SPAWN_SPIDERLING_TM_OPENING = 99;   // opening drop: almost full TM
...
export function makeSpiderContent({ stageNumber, boss, spawnTemplate, difficulty = 'Normal' }) {
  ...
  const spawn = (state, n, tm = SPAWN_SPIDERLING_TM_SUBSEQUENT) => {
    ...
    for (let i = 0; i < k; i++) {
      ...
      state.enemies.push(makeCombatant({ name: `Spiderling#${(state.spiderSeq = (state.spiderSeq ?? 0) + 1)}`,
        ...
        turnMeter: tm }));   // opening drop enters at ~full TM; all later drops at 0 TM (Mike first-party 2026-07-29)
    }
  ...
  function spiderlingAct(state, sp) {
    ...
    const pel = state.allies.find((a) => champKey(a.name) === champKey('Pelops the Victor'));
    ...
    recordEffect(state, { kind: 'spider_pick', subtype: 'targeting', actor: sp.name, target: target.name,
      reason: (target.buffs ?? []).some((b) => b.type === 'Taunt' || b.type === 'Provoke') ? 'TAUNT' : 'LOWEST_MAX_HP',
      pelTauntable, pelTaunting: pelTauntable && (pel.buffs ?? []).some((b) => b.type === 'Taunt' || b.type === 'Provoke'),
      pelPicked: !!pel && champKey(target.name) === champKey('Pelops the Victor') });
  ...
  return {
    ...
    onPhaseStart(state) { spawn(state, SPAWN.onRoundStart, SPAWN_SPIDERLING_TM_OPENING); },       // opening 6 enter at ~full TM (immediate turn)
