...
const SPAWN_SPIDERLING_TM = 66;       // Spiderlings enter at ~2/3 Turn Meter (Mike first-party 2026-07-29), not 0 — TUNABLE (calibrate vs reality first-tick rate)
  ...
  const spawn = (state, n) => {
    ...
    for (let i = 0; i < k; i++) {
      ...
      state.enemies.push(makeCombatant({ name: `Spiderling#${(state.spiderSeq = (state.spiderSeq ?? 0) + 1)}`,
        ...
        turnMeter: SPAWN_SPIDERLING_TM }));   // Spiderlings drop with ~2/3 TM, NOT 0 (Mike first-party 2026-07-29) — decides the burn tick-vs-consume race
    }
