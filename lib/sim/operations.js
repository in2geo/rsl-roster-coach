...
export const OPERATIONS = {
  ...
  REVIVE_AND_CAST: { implemented: true, cat: 'state',   // Iudex Artor A3 "Revival Mandate" (+ A4 "Sentenced to Life"): revive one ally, buff it, cast its default skill at the lowest-HP enemy, reset own cd on the kill (interpreter.doReviveAndCast)
    validate: (a) => (a.effect?.hpPct == null ? ['effect.hpPct'] : []) },
  ...
  BOOST_SHIELD: { implemented: true, cat: 'buff',   // raise [Shield] value by pctOfCasterMaxHp × count (Bambus A2)
    validate: (a) => (a.effect?.pctOfCasterMaxHp == null ? ['effect.pctOfCasterMaxHp'] : []) },
  PLACE_BOMB: { implemented: true, cat: 'debuff',   // [Bomb]: detonation damage = multiplier × caster ATK, after `countdown` turns (Ezio A2)
    validate: (a) => (a.effect?.multiplier == null ? ['effect.multiplier'] : []) },
  ...
  BUFF_STRIP:           { implemented: true, cat: 'buff', validate: () => [] },   // "Buff Strip": delete buffs from enemies (interpreter.doBuffStrip). types/count optional; default = all. Distinct from STEAL_BUFF (takes them) + CLEANSE (ally debuffs).
  ...
  STEAL_TURN_METER:     { implemented: true, cat: 'turn-meter', validate: (a) => (a.effect?.pct == null ? ['effect.pct'] : []) },   // drain enemy TM + fill caster (interpreter.doStealTurnMeter)
  RESET_COOLDOWN:       { implemented: false, cat: 'cooldown' },
  KILL:                 { implemented: false, cat: 'state' },
  ALLY_ATTACK:          { implemented: true, cat: 'trigger',   // "Ally Attack" / join-attack: allies each attack the acquired target with their default skill (interpreter.doAllyAttack)
    validate: (a) => (a.who && !['all_allies', 'all_allies_except_self', 'random_ally_except_self', 'random_faction_ally'].includes(a.who) ? [`who:${a.who}`] : []) },
  COUNTERATTACK:        { implemented: true, cat: 'trigger', validate: () => [] },   // on-attacked reaction: re-run a skill (default A1) at the attacker (Ezio P2)
  REDIRECT_DAMAGE:      { implemented: false, cat: 'mitigation' },
  SPREAD_DEBUFFS:       { implemented: true, cat: 'debuff', validate: () => [] },   // Michelangelo A2 "Express Delivery!": copy all debuffs on the primary target onto every other enemy (interpreter.doSpreadDebuffs)
  SELF_RECOVER_ON_BURN: { implemented: true, cat: 'state', validate: () => [] },    // Artak A3: restore destroyed MAX HP per burn placed + heal per burn resisted (interpreter.SELF_RECOVER_ON_BURN)
  ...
  EQUALIZE_HP:          { implemented: true, cat: 'state', validate: () => [] },   // raise all allies to the highest ally's HP fraction (interpreter.doEqualizeHp) — Mavara A2
  SWAP_HP:              { implemented: true, cat: 'state', validate: () => [] },   // exchange caster/target HP fractions (interpreter.doSwapHp) — Vallaryn A3
  BUFF_ACTIVATION:      { implemented: true, cat: 'buff', validate: () => [] },     // force an ally [Continuous Heal] to tick now (interpreter.doBuffActivation) — Donatello A1
