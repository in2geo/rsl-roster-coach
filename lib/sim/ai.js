 ...
 * to land at the right trigger. v0 recognises the two we can place faithfully:
 *   startOfTurn   — "...at the start of each turn" (Perfect Veil renewal, self-buffs)
 *   startOfBattle — "at the start of the battle ..."
 * Every other real trigger (on-hit, on-death, on-crit, on-kill, each round) returns null — the engine
 * FLAGS such a passive as unmodelled rather than firing it at a guessed moment. Match the mechanic,
 * not the word: a passive with no trigger phrase is null too, so nothing fires blind.
 */
...
export function readSkillKit(rows = []) {
  ...
  for (const r of rows) {
    ...
    const placeable = t.replace(/(?:will +)?(?:ignores?|removes?|steals?|strips?)[^.]*[.]/gi, ' ');
    ...
    const s = {
      ...
      coeffStat: normStat(r.multiplier_type) ?? cc?.stat ?? 'atk',
      maxHpPct,
...
export function parseCoeff(v) {
  ...
  if (!t) return null;
  const atk = t.match(/([\d.]+)\s*ATK/i);
  if (atk) return { value: +atk[1], stat: 'atk' };                 // ATK term wins (resolves the compound form too)
  if (/^[\d.]+$/.test(t)) return { value: +t, stat: 'atk' };       // bare number defaults to ATK
  if (!/MAX\s*HP/i.test(t)) { const hp = t.match(/([\d.]+)\s*HP\b/i); if (hp) return { value: +hp[1], stat: 'hp' }; }
  const def = t.match(/([\d.]+)\s*DEF\b/i); if (def) return { value: +def[1], stat: 'def' };
  return null;                                                     // e.g. pure "0.02 Enemy MAX HP" — %maxHP, read by parseMaxHpPct
}
