...
// CONFIRMED per-champion non-default skill order (knowledge/CHAMPION_AI_MODEL.md). Keyed by the canonical
// champions.name. ONLY confirmed-vs-real-battle exceptions live here; SUSPECTED (community-sourced) ones
// stay in the follow-up queue and are NOT applied. Default (no entry) = highest slot first (A3>A2>A1).
export const CONFIRMED_SKILL_ORDER = {
  'Ezio Auditore': ['A2', 'A3', 'A1'],   // opens A2 (Da Vinci's Design) in the Perfect Veil window — confirmed 2026-07-23
  // Hilvi opens A2 (Embittering Cold, freeze-all) every cooldown, then A3 (Ward of the Glacier) for the
  // proactive [Increase SPD]+TM-fill, A1 last — game AI is control-first (confirmed Hilvi AI 2026-08-01).
  'Hilve the Rime-called': ['A2', 'A3', 'A1'],
  'Hilvi': ['A2', 'A3', 'A1'],
  // Coldheart's default AI prioritizes her A2 AoE OVER her A3 boss nuke (the notorious "Coldheart AI is
  // horrible" behavior) — so A3 (Heartseeker, −100% TM drain) first fires on turn 2, not turn 1, and never
  // wastes on the low-TM opening boss. Confirmed 2026-08-01 (Plarium forum + gameplay).
  'Coldheart': ['A2', 'A3', 'A1'],
  // Ninja: A2 (Hailburn) > A3 (Cyan Slash) > A1 (Shatterbolt) — FULLY confirmed 2026-08-02 (Mike first-party
  // + reviewer AI table: A2 used first when available, A3 when A2 is on cooldown, A1 as fallback). NOT the
  // default A3-first. Hailburn is his HP-Burn engine (+ [Perfect Veil] self-buff) and the bulk of his real
  // damage; front-loading it fixed his under-damage (sim dealt 522k → 595k, reality 613k). A3 is a minor AoE.
  'Ninja': ['A2', 'A3', 'A1'],
};
...
export function canUseSkill(s, state, actor) {
  ...
  switch (classifySkill(s)) {
    case 'revive':       return anyDead || !!s.proactiveBuffs;  // STRICT lock, EXCEPT utility-revives whose card grants buffs "even if no allies were revived" (Hilvi) — cast proactively for the SPD/TM buff
...
export function readSkillKit(rows = []) {
  ...
  for (const r of rows) {
    ...
    // UTILITY REVIVE (e.g. Hilvi's Ward of the Glacier): the card explicitly grants its ally buffs
    // "even if no allies were revived" — so the AI casts it PROACTIVELY for the buff, not only on a death.
    // This is the game-authored signal that the revive-lock must NOT hold. (Confirmed Hilvi AI 2026-08-01.)
    const proactiveBuffs = revives && /even if no allies (?:were|are) revived/i.test(t);
    ...
    for (const [re, type] of [
      ...
      [/(\d+)%\s*\[Increase SPD\]/i, 'Increase SPD'], [/(\d+)%\s*\[Increase C\.?\s*RATE\]/i, 'Increase C.RATE'], [/\[Shield\]/i, 'Shield'],
    ...
    for (const [re, type, extra] of [
      ...
      [/(\d+)%\s*\[Weaken\]/i, 'Weaken', {}],
      [/(\d+)%\s*\[Decrease SPD\]/i, 'Decrease Speed', {}],
      [/(\d+)%\s*\[Decrease Speed\]/i, 'Decrease Speed', {}],
    ...
    // CROWD CONTROL (Stun / Sleep / Freeze / Petrification). The engine already CONSUMES these — a champ
    // with one loses its turn (CC_SKIPS_TURN) — but readSkillKit never PARSED the placement, so mobs never
    // applied CC. That is Dragon's defining wall (INS-0021) and the stage-17 gap (Tayrel/Crossbowman).
    // ISOLATED from the debuff loop above so existing parsing (and the frozen stage-16 golden, whose mobs
    // place no CC) is byte-for-byte untouched. Per CC: the PLACEMENT CHANCE ("50% chance of placing…") and
    // DURATION ("for N turns") are read from the local clause, so a 50% stun rolls (two-stage: chance →
    // ACC/RES) instead of always landing. CONDITIONAL placements ("If <cond>, … place a [Stun]" — Tayrel A2
    // Sleep-if-Decrease-ATK, A3 Stun-if-TM-depleted) are DEFERRED, not over-applied: the sim can't evaluate
    // the condition, so the safe choice is not to place it (a conditional-placement engine is a follow-on).
    // TURN METER manipulation (instant, not a lasting status). "Decrease/Deplete Turn Meter by X%" → the
    // attack TARGET(s) (harmful, ACC/RES-gated in the engine); "Fill/Increase Turn Meter by X%" → OWN side.
    // Tayrel A3 decreases 50%, Lua A3 100%; Apothecary A3 fills all allies 15%. Direction picks the side.
    const tmDec = t.match(/(?:decreases?|depletes?)[^.]*?turn meter[^.]*?by (\d+)%/i);
    const tmInc = t.match(/(?:fills?|increases?)[^.]*?turn meter[^.]*?by (\d+)%/i);
    const turnMeterEnemy = tmDec ? +tmDec[1] : null;
    const turnMeterAlly = tmInc ? +tmInc[1] : null;
    const turnMeterAllySelf = turnMeterAlly != null && /turn meter of this champion/i.test(t);

    // EXTRA TURN — the SAME champion acts again. "Grants an Extra Turn." is UNCONDITIONAL (Hordin A3,
    // Crossbowman A2); "Grants an Extra Turn if the target is killed" is on-kill (Hordin A1). The "if"
    // clause distinguishes them so a conditional grant isn't fired unconditionally.
    const grantsET = /grants? (?:an? )?extra turn/i.test(t);
    const etConditional = /extra turn\s+if\b/i.test(t);
    const extraTurn = grantsET && !etConditional;                       // unconditional
    const extraTurnOnKill = grantsET && etConditional && /kill/i.test(t);

    const ccText = placeable.replace(/\bif\b[^.]*?place[^.]*?\[(?:Stun|Sleep|Freeze|Petrification)\][^.]*\./gi, ' ');
    for (const cc of ['Stun', 'Sleep', 'Freeze', 'Petrification']) {
      const idx = ccText.search(new RegExp('\\[' + cc + '\\]', 'i'));
      if (idx < 0) continue;
      const clause = ccText.slice(ccText.lastIndexOf('.', idx) + 1, (ccText.indexOf('.', idx) + 1) || ccText.length);
      const chance = clause.match(/(\d+)%\s*chance/i);
      const dur = clause.match(/for (\d+) turn/i);
      debuffs.push({ type: cc, cc: true, value: null, pct: null,
        chance: chance ? +chance[1] / 100 : null, turns: dur ? +dur[1] : 1 });   // CC defaults to 1 turn when unstated
    }

    // CONDITIONAL PLACEMENT — "If <cond>, (it will) place a [Y] debuff for N turns" (Tayrel A2 Sleep-if the
    // target is under [Decrease ATK]; A3 Stun-if the attack fully depletes the Turn Meter). Deferred by the
    // CC strip above; parsed here WITH the condition so the engine places [Y] only when it holds — the two
    // condition kinds we can evaluate: target under a named debuff, and target Turn Meter fully depleted.
    const DEBUFF_ALIAS = { 'Decrease ATK': 'Decrease Attack', 'Decrease DEF': 'Decrease Defense' };
    const conditionalDebuffs = [];
    for (const m of t.matchAll(/\bif\s+([^,]+?),\s*(?:it will\s+)?place[^.]*?\[([^\]]+)\][^.]*?(?:for (\d+) turn)?/gi)) {
      const condText = m[1], effect = m[2].trim(), turns = m[3] ? +m[3] : 1;
      const underM = condText.match(/under (?:a |an )?\[([^\]]+)\]/i);
      let condition = null;
      if (underM) condition = { kind: 'under_debuff', debuff: DEBUFF_ALIAS[underM[1].trim()] ?? underM[1].trim() };
      else if (/deplete[sd]?\s+the\s+turn\s+meter|turn\s+meter\s+(?:is|reaches)\s+(?:0|zero|empty)/i.test(condText)) condition = { kind: 'tm_depleted' };
      if (condition) conditionalDebuffs.push({ type: effect, turns, condition });
    }
    ...
    const s = {
      slot, name: r.skill_name, isPassive, passiveTrigger, cooldown: num(r.cooldown_base) ?? 0, cooldownBooked: num(r.cooldown_booked), cdLeft: 0,
      ...
      maxHpPct,
      hitsEnemies, aoe, revives, proactiveBuffs, cleanses,
      healPct: healMatch ? +healMatch[1] / 100 : null,
      aoeHeal: /heals? all allies/i.test(t),
      buffs, debuffs,
      turnMeterEnemy, turnMeterAlly, turnMeterAllySelf,   // instant Turn Meter manipulation (engine applies)
      extraTurn, extraTurnOnKill,                          // the actor acts again (engine turn loop grants it)
      conditionalDebuffs,                                  // "if <cond>, place [Y]" — engine evaluates the condition
      unread: [],
    };
