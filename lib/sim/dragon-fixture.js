...
// flat bonus. The SPD aura adds a % of BASE speed ONLY — leader auras never scale geared/gear speed
// (True Speed §4) — so it reads `ally.baseSpd` (wired from champions.base_spd above). Arena is a flat % on
// HP/ATK/DEF (no SPD/ACC/RES). Centralized here so sim-fixture-volume / sim-run / model-golden share ONE
// formula and cannot drift. Default aura +19% (Ezio), arena ×1.03 (Bronze III).
export function applyBattleLayers(allies, { auraSpdPct = 0.19, arena = 1.03 } = {}) {
  for (const a of allies) {
    a.spd = a.spd + Math.round((a.baseSpd ?? 0) * auraSpdPct);   // + % of BASE speed, added on top of the geared total
