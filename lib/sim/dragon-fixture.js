...
// STATIC Ignore-DEF from COMPLETE gear sets (deterministic set effects, data/gear-set-procs.json). A 4-piece
// set strips that fraction of the target's DEF on every hit — a per-attacker mechanic BOSS_DEF_FACTOR was
// smearing globally. Only complete sets count (a loose "Savage×1" grants nothing). Multiple sets are additive.
const IGNORE_DEF_SETS = { savage: 0.25, lethal: 0.25, instinct: 0.20, cruel: 0.05, swiftparry: 0.50 };
function ignoreDefFromSets(gearSets) {
  let total = 0;
  for (const g of gearSets || []) {
    const m = String(g).match(/^\s*(.+?)\s*[×x*]\s*(\d+)/);   // "Savage×4" → name, count
    const key = (m ? m[1] : String(g)).toLowerCase().replace(/[^a-z]/g, '');
    const frac = IGNORE_DEF_SETS[key];
    if (frac != null && (m ? +m[2] : 0) >= 4) total += frac;   // 4-piece = complete
  }
...
export async function buildBattle({ rest, fixture, repoRoot }) {
  ...
  const allies = (fixture.team || []).map((shortName) => {
    ...
    const ally = makeCombatant({ name: shortName, side: 'ally',           // SHORT name aligns with the fixture record
      ...
      ignoreDefStatic: ignoreDefFromSets(b.gear_sets || []),             // STATIC gear Ignore DEF from COMPLETE 4-sets (Savage/Lethal/Instinct/Cruel/Swift Parry) — consumed in computeRawHit + applyWarmaster
      // slots whose skill is FULLY BOOKED (maxed) — gates per-skill book damage (F.bookDamage) in the interpreter.
      // ⚠ LIMITATION: this is BINARY (fully-booked → full book%; anything less → 0). It does NOT model PARTIAL
      // booking (e.g. Hilvi A2/A3 at 2/3) — that needs ORDERED per-level upgrade data (which upgrade is Damage +%
      // at which level) to compute a partial book%, which the DB does not have. Currently only Michelangelo A1
      // carries bookDamage AND is fully booked, so the approximation does not bite; revisit when adding bookDamage
      // to a skill that captured accounts run partially booked (see knowledge/SKILL_BOOK_DAMAGE_CAPTURE.md).
      bookedSlots: new Set(Object.entries(b.skill_levels || {}).filter(([, v]) => v?.maxed).map(([s]) => s)),
