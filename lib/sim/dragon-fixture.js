...
export async function buildDragonBattle({ rest, fixture, repoRoot }) {
  ...
  const allies = (fixture.team || []).map((shortName) => {
    ...
    // Lifesteal 4-set = heals 30% of damage dealt (game fact, verbatim in the build's gear_sets). The
    // builder discarded this — the exact sustain the fixture flagged as dropped (Pelops tanks the waves
    // via Magma-Shield-reflect-into-lifesteal, which heals nothing at lifesteal 0).
    const lifesteal = (b.gear_sets || []).some(g => /lifesteal/i.test(g)) ? 0.30 : 0;
