...
export async function buildBattle({ rest, fixture, repoRoot }) {
  ...
  const allies = (fixture.team || []).map((shortName) => {
    ...
    const ally = makeCombatant({ name: shortName, side: 'ally',           // SHORT name aligns with the fixture record
      ...
      critRate: s.crit_rate, critDmg: s.crit_dmg, affinity: b.affinity ?? cat?.affinity, rarity: b.rarity ?? cat?.rarity ?? null, lifesteal,   // rarity → Clan Boss HP-burn cap (50k Rare / 75k Epic+)
...
export function applyBattleLayers(allies, { auraSpdPct = 0.19, arenaPct = 0.03, accAura = 0 } = {}) {
