...
// ONE normalizer for the whole app — the shared registry normalizer (lib/champion-names.js),
// replacing a local `toLowerCase().replace(...)`. Named champions in the SYNERGIES config below
// (`champion`, `set`) must be canonical `champions.name` values: these rules carry no DB resolver,
// so alias/short forms can't be resolved to an id here — the config side is matched by name only.
