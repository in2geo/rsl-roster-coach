...
// ── Level stat scaling ──────────────────────────────────────────────────────────
// Champion base stats in the DB are the 6★ / level-60 MAX. Only HP/ATK/DEF grow with level —
// SPD/ACC/RES/crit are fixed per champion, level-independent. Returns the fraction of the max
// HP/ATK/DEF a champion has at `level`. Calibrated from Gestal real stats (low-gear champs:
// ~0.20 at lvl 1 → ~0.39 at lvl 40 → 1.0 at lvl 60); a power curve with a level-1 floor.
// MANUAL-roster estimator only — Gestal rosters carry real effective_stats and never call this.
// `stars` is reserved for a future ascension refinement (level already correlates with stars).
export function levelStatScale(level, stars = null) {
  const L = Math.max(1, Math.min(60, Number(level) || 60));
  return 0.20 + 0.80 * Math.pow((L - 1) / 59, 3.5);
}
...
// ── Affinity matchup vs the boss (dungeon team scoring) ───────────────────────
// GAME FACT (like the damage-mechanics rules): affinity disadvantage causes "Weak
// Hits" — reduced damage with crits suppressed — and lets the enemy crit the weak
// champ more. So a WEAK champ both deals less (kill-speed side) AND takes more
// (survival side); a STRONG champ does the reverse. Void is neutral either way, and
// same-affinity is neutral. This is a two-sided factor, mirroring the contribution model.

// 'strong' | 'weak' | 'neutral' for a champion attacking / defending vs a boss affinity.
export function affinityMatchup(champion_affinity, boss_affinity) {
  if (!champion_affinity || !boss_affinity) return 'neutral';
  if (champion_affinity === 'Void' || boss_affinity === 'Void') return 'neutral';
  if (AFFINITY_BEATS[champion_affinity] === boss_affinity) return 'strong'; // champ beats boss
  if (AFFINITY_BEATS[boss_affinity] === champion_affinity) return 'weak';   // boss beats champ
  return 'neutral';
}

// Nominal, UNCALIBRATED multipliers (game-knowledge magnitudes, not battle-tuned — the
// weak-hit chance / crit-suppression is a probability, so these are conservative expected
// values). Offense = the champ's damage/debuff-landing; survival = how well it takes boss
// hits. Weak hurts both; strong helps both; neutral = 1.0. Calibrate against captured
// runs at known-affinity stages (e.g. DonBrogni Spider 11 Force near-loss).
export const AFFINITY_FACTORS = {
  strong:  { offense: 1.20, survival: 1.10 },
  weak:    { offense: 0.70, survival: 0.85 },
  neutral: { offense: 1.00, survival: 1.00 },
};

export function affinityFactor(champion_affinity, boss_affinity, side = 'offense') {
  return AFFINITY_FACTORS[affinityMatchup(champion_affinity, boss_affinity)][side];
}
