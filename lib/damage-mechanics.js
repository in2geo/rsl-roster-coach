...
// ---------------------------------------------------------------------------
// 6b. %MAX-HP DAMAGE IS HARD-CAPPED IN BOSS CONTENT — a STAGE-INDEXED CLIFF.
// ---------------------------------------------------------------------------
// Plarium balances %-of-enemy-MAX-HP skills against boss content with a hard per-hit
// ceiling: an active Enemy-MAX-HP skill can deal at most a fixed FRACTION of the boss's
// MAX HP per hit, regardless of what the skill prints. A skill reading 25% delivers 10%.
//
// WHY THIS MATTERS STRUCTURALLY (2026-07-21). The cap does NOT restore HP-sensitivity —
// it is itself expressed as a fraction of MAX HP, so it still cancels out of
// `killTurns = bossHp / perTurn` exactly as an uncapped %maxHP source does (see the
// contribution model). What it produces is better: a STEP CHANGE IN THE COEFFICIENT at a
// stage boundary. Crossing into Normal 21 can cut a MaxHP-nuke team's output ~2.5x in one
// step. That is a genuine, sharp, stage-indexed discontinuity — the shape every other
// proxy in this project lacks (stat floors ramp ~1pt/stage; the pool grade sits at 75-98%
// for every tier). It is the first modelled mechanism that can express a WALL.
//
// CORROBORATION: `lib/spider-rubric.js` declares the `maxhp_nuke` strategy as
// `stages: [15, 20]` — it ENDS exactly where this cap begins. Those tier boundaries came
// from a hand-read community guide and were removed as opinion on 2026-07-21; the 21
// boundary specifically appears to have a real mechanical basis after all. (The 15
// boundary remains unsourced.) No account in the corpus has cleared Spider 21+.
//
// ⚠ SOURCING — INHERITED, NOT VERIFIED. Supplied by Mike 2026-07-21 from a community
// thread. Factual game mechanics are permitted as a source under CLAUDE.md (the
// restriction is on EDITORIAL content — tier lists, build guides), and per the
// damage-mechanics convention a published mechanic MAY become a model rule. But the exact
// 10% / 5% figures are ONE unconfirmed citation: treat the EXISTENCE of a boss cap as the
// fact and the NUMBERS as directional until an in-game observation or Plarium patch note
// confirms them. Do not calibrate anything else against these magnitudes meanwhile.
export const MAXHP_DAMAGE_CAP = {
  none:  null,  // Normal dungeon stages 1-20 — no cap applies
  boss:  0.10,  // Normal 21-25, Hard Mode dungeons, Doom Tower bosses, Hydra, Iron Twins,
                // Sand Devil, Shogun — max 10% of boss MAX HP per hit
  ultra: 0.05,  // certain ultra-late content — 5% per hit (content list not yet enumerated)
};

// Which sources the cap applies to. The source describes "active Enemy MAX HP SKILLS", so
// it is scoped to `enemy_maxhp` ONLY.
// ⚠ UNVERIFIED for the DoT family: whether Poison / HP Burn ticks and Warmaster / Giant
// Slayer procs are also capped is NOT established. Left uncapped deliberately — over-
// applying a cap would under-credit DoT teams, the mirror of the §1 over-credit trap, and
// our nominal coefficients (poison/hp_burn 0.025, warmaster 0.024) sit far under 10% so it
// would not bind today regardless. Revisit if real per-skill percentages ever exceed it.
export const MAXHP_CAP_APPLIES_TO = new Set(['enemy_maxhp']);

/**
 * The cap tier for a piece of content. Normal dungeon stages 21-25 and Hard mode are
 * capped; everything else we model today is not.
 * @returns {number|null} max fraction of enemy MAX HP per hit, or null for uncapped
 */
export function maxHpCapFor({ stageNumber = null, difficulty = null, contentKey = null } = {}) {
  if (String(difficulty ?? '').toLowerCase() === 'hard') return MAXHP_DAMAGE_CAP.boss;
  if (contentKey === 'clan_boss') return null;           // CB is its own damage race, uncapped here
  if (stageNumber != null && stageNumber >= 21) return MAXHP_DAMAGE_CAP.boss;
  return MAXHP_DAMAGE_CAP.none;
}

/**
 * Clamp one source's per-hit fraction-of-MAX-HP to the content cap.
 * No-op for sources the cap does not cover, and for uncapped content.
 * @param {string} source  a DAMAGE_SOURCES key
 * @param {number} fraction  nominal fraction of enemy MAX HP this source deals per hit
 * @param {number|null} cap  from maxHpCapFor()
 */
export function applyMaxHpCap(source, fraction, cap) {
  if (cap == null || !MAXHP_CAP_APPLIES_TO.has(source)) return fraction;
