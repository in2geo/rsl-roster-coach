...
/**
 * Reads a normalized Gestal account export. With no accountId, picks the most
 * recently synced file in the output dir. Returns null if none exists.
 */
export function readGestalRoster(accountId = null, dir = GESTAL_OUTPUT_DIR) {
  ...
  try {
    ...
    return applyAscensionOverrides(roster);
  } catch {
    return null;
  }
}

// Interim ascension source: Gestal zeroes champion ascensionLevel in its export
// (KNOWN_GAPS.md), so ascension-gated tags never count. This merges an account-local
// override file into the roster at read time — ONLY where the Gestal value is 0, so a
// real non-zero value (from Option B, once wired) supersedes the override silently.
// Writes to `ascensionLevel` (camelCase) — the field buildUserChampions maps to
// uc.ascension_level, which the mapRoster ascension gate reads.
const ASCENSION_OVERRIDES_FILE = path.join(REPO_ROOT, 'config', 'ascension_overrides.json');
let _ascensionOverrides; // Map(normName → level), lazy-loaded once

function loadAscensionOverrides() {
  if (_ascensionOverrides) return _ascensionOverrides;
  _ascensionOverrides = new Map();
  try {
    const { overrides } = JSON.parse(fs.readFileSync(ASCENSION_OVERRIDES_FILE, 'utf8'));
    for (const o of overrides ?? [])
      if (o?.champion_name != null && o?.ascension_level != null)
        _ascensionOverrides.set(normName(o.champion_name), o.ascension_level);
  } catch { /* no override file → empty map */ }
  return _ascensionOverrides;
}

export function applyAscensionOverrides(roster) {
  const ov = loadAscensionOverrides();
  if (!ov.size || !Array.isArray(roster?.champions)) return roster;
  for (const c of roster.champions) {
    if (!c.ascensionLevel) {                       // 0/absent only — real values win
      const lvl = ov.get(normName(c.name));
      if (lvl != null) c.ascensionLevel = lvl;
    }
  }
  return roster;
}
...
export function buildUserChampions(gestalChampions = [], dbChampions = []) {
  ...
  for (const g of gestalChampions) {
    ...
    // baseTypeId is the stable champion id (per-copy typeId = baseTypeId + a 0-9
    // copy index); match on it, then name.
  ...
  const userChampions = [...bestById.values()].map(({ gestal: g, db }) => ({
    ...
    level:           g.level ?? 1,
    stars:           g.stars ?? 1,
    ascension_level: g.ascensionLevel ?? 0,
