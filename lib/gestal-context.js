...
import fs from 'fs';
import { normalizeName, buildNameResolver } from './champion-names.js';
import path from 'path';
import { fileURLToPath } from 'url';
import { effectiveStats } from './effective-stats.js';
import { hasBossMastery, masteryNames } from './masteries.js';
...
/**
 * GRADED book progress in [0,1] — the fix for `fullyBooked` being a cliff (INS-0033 known error).
 * A champion 3/4 on one skill and maxed on the rest (Michelangelo) is ~92% booked; one at 1/5, 1/6,
 * 1/5 (Ninja) is ~19%. The binary flag charged BOTH the full unbooked penalty. This sums actual book
 * levels over their maxima across BOOKABLE skills only (maxLevel > 1 — passives/A-skills without books
 * neither help nor hurt). Returns null when there is no skill data at all (capture gap ⇒ neutral, never
 * a penalty), matching the file's "absence ⇒ 1.0" convention. Owned champions carry real per-skill
 * levels in the Gestal export, so this reads authoritatively — no player question needed.
 */
export function bookFraction(g) {
  const skills = g?.skills;
  if (!Array.isArray(skills) || skills.length === 0) return null;
  const bookable = skills.filter(s => Number(s?.maxLevel ?? 1) > 1);
  if (!bookable.length) return null;                       // no bookable skills ⇒ no signal
  let cur = 0, max = 0;
  for (const s of bookable) {
    const mx = Number(s.maxLevel);
    cur += Math.min(Number(s.level ?? 0), mx);
    max += mx;
  }
...
export function buildUserChampions(gestalChampions = [], dbChampions = [], dbAliases) {
  ...
  // Name fallback delegates to the ONE registry (buildNameResolver: every champions.name + champion_aliases
  // normalized to one key space, canonical winning on a collision) instead of re-implementing that map here.
  // resolve() → {id,name}; dbById maps that id back to the full champion row this function needs.
  const nameResolver = buildNameResolver(dbChampions, dbAliases);
  ...
  for (const g of gestalChampions) {
    ...
    const db = dbByTypeId.get(g.baseTypeId ?? g.typeId) ?? dbById.get(nameResolver.resolve(g.name)?.id);
  ...
  const userChampions = [...bestById.values()].map(({ gestal: g, db }) => ({
    ...
    // FULL decoded mastery NAMES (data/masteries.json). has_boss_mastery above is the coarse boolean the
    // PRODUCT's boss-damage estimate needs; the SIMULATOR wants the whole list so engine.deriveMasteryMech
    // can model the target-type-aware Warmaster/Giant Slayer boss bonus AND the offense-proc damage masteries
    // (Heart of Glory, Bring It Down, …). Flat-stat masteries are NOT re-applied downstream — they are already
    // in effective_stats via bonusesV2.mastery — so this only adds the proc/%maxHP mechanics. [] for manual rosters.
    masteries: masteryNames(g.masteryIds),
    ...
    // GRADED book progress — buildScale reads this to charge a partial book penalty (Michelangelo
    // 3/4 ≈ full, Ninja 1/x ≈ none) instead of the binary cliff above. null ⇒ no data ⇒ neutral.
    book_fraction:   bookFraction(g),
...
export function buildContext({ gestalRoster, userChampions, unmatched, battleLog }) {
  ...
  const rarityByName = new Map(owned.map(c => [normName(c.name), c.rarity]));
  const untaggedRarePlus = (unmatched ?? [])
    .filter(n => SURFACED_RARITIES.has(rarityByName.get(normName(n))));
