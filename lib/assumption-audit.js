...
// Match CAPTURED battle hero names (external) to the engine's team. This function is pure and
// receives no DB resolver, so it can only unify punctuation/case/accents via the shared registry
// normalizer (normalizeName) — it CANNOT resolve aliases/short forms to a champions.id. A captured
// name that differs from champions.name by more than punctuation (an alias) will still miss here;
// callers with a resolver should prefer buildRosterIndex (see tools/reconcile-runs.mjs). ev.team
// objects carry `.id`, but the lookup key must be the external name, so this is name-keyed.
import { normalizeName as norm } from './champion-names.js';
