import BLESSING_BONUS from '../data/blessing-bonus-stats.json' with { type: 'json' };
 ...
 */
/**
 * BLESSING BONUS STATS — the awakening term the model never had (Patch 8.20).
 *
 * A champion with a Blessing assigned gains flat stats purely as a function of
 * (rarity x awakening level), CUMULATIVE through their current level. This is NOT the
 * blessing's ability (Iron Will, Indomitable Spirit, ...) — that is a separate, unmodelled
 * layer. Source: Plarium in-game Help, supplied 2026-07-21; values in
 * data/blessing-bonus-stats.json.
 *
 * WHY IT MATTERS: Mike flagged 2026-07-21 that awakening entered no formula at all. It was
 * carried through five files and dropped. A 6-star-awakened Legendary is worth HP +8500,
 * ATK +750, DEF +600, C.DMG +38%, RES +75, ACC +75, SPD +15 — against fielded-team HP of
 * 10k-28k and a Spider ACC floor of stage x 10, that is enormous and we counted none of it.
 *
 * GATED on a blessing being assigned. Both inputs (rarity, awakenLevel) are captured at 100%,
 * so this needs no new data — it was purely unimplemented.
 *
 * @returns {{flat:object, cdmgPct:number}} flat stat adds + a C.DMG PERCENTAGE
 */
export function blessingBonus(champ) {
  const out = { flat: { hp: 0, atk: 0, def: 0, spd: 0, res: 0, acc: 0 }, cdmgPct: 0 };
  const hasBlessing = champ?.blessingId != null && champ.blessingId !== 0;
  const lvl = Number(champ?.awakenLevel ?? 0);
  if (!hasBlessing || !(lvl > 0)) return out;
  const rows = BLESSING_BONUS.by_rarity?.[champ?.rarity];
  if (!rows) return out;                      // unknown rarity → credit nothing
  for (const r of rows) {
    if (r.level > lvl) continue;              // CUMULATIVE: every level up to the current one
    for (const k of ['hp', 'atk', 'def', 'spd', 'res', 'acc']) if (r[k]) out.flat[k] += r[k];
    if (r.crit_dmg_pct) out.cdmgPct += r.crit_dmg_pct;
  }
  return out;
}
...
export function effectiveStats(champ) {
  ...
  // Blessing bonus stats are FLAT adds on top of gear (they are not gear % bonuses, so they
  // must not be multiplied by the base). C.DMG is the one percentage in the table.
  const bless = blessingBonus(champ);

  const num = (k) => Math.round(base[k] ?? 0);
  const withPct = (k) => num(k) + Math.round((base[k] ?? 0) * (pct[k] ?? 0) / 100) + flat[k];
  const effective = {
    hp:    withPct('hp')  + bless.flat.hp,
    atk:   withPct('atk') + bless.flat.atk,
    def:   withPct('def') + bless.flat.def,
    spd:   withPct('spd') + bless.flat.spd,
    crate: num('crate') + flat.crate,
    cdmg:  num('cdmg')  + flat.cdmg + bless.cdmgPct,
    res:   num('res')   + flat.res  + bless.flat.res,
    acc:   num('acc')   + flat.acc  + bless.flat.acc,
  };

  return { effective, breakdown: { base, flat, pct, sets, applied, blessing: bless } };
}
