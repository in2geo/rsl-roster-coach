 ...
 *
 * SHAPE: floored at BUILD_MIN so a leveled-but-unstarred champ still delivers ~half, NEVER 0 (the
 * file's "absence ⇒ 1.0, never 0" convention). Gentle: L60/6★ → 1.0, well-built L50/4★ → ~0.91,
 * L30/3★ → ~0.80 — a ~9% edge between a maxed L60 and a solid L50, so ROLE-FIT dominates level.
 */
export const BUILD_MIN = 0.5;
export function buildScale(champ) {
  ...
  const s = 0.60 * Math.min(1, (lvl ?? 0) / 60)
          + 0.40 * Math.min(1, (str ?? 0) / 6);
  return BUILD_MIN + (1 - BUILD_MIN) * s;
}
