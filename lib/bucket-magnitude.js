...
/**
 * BUILD SCALE — the 4th term of the INS-0031 model (`effect × uptime × land rate × BUILD SCALE`),
 * finally implemented. How much champion is PERMANENTLY behind this effect, keyed on SUNK investment
 * only: level and stars. GEAR IS DELIBERATELY EXCLUDED (2026-08-01, Mike): gear is PORTABLE between
 * champions, so penalising a champion for the gear they happen to wear TODAY would hide a level-ready
 * champion who would perform once geared — the model recommends on POTENTIAL and surfaces "gear this
 * seat" as an action instead. Excluding gear also keeps this ORTHOGONAL to `landRate`, which already
 * scales on ACC (a gear-derived stat) — so the two never double-count the same portable axis.
 *
 * WHY THIS EXISTS (Mike, 2026-08-01): "the most developed is the starting point, not the end point...
 * it should not be a lockout." Development used to be a SEED + BUILD_FLOOR veto OUTSIDE the objective
 * precisely because build scale was missing here — an ungeared L40 healer filled `sustain` to the
 * same 100% as a maxed one. With build scale IN the delivery, development becomes MAGNITUDE (a mild
 * bias), so a better-built champion delivers more but a lower-level champion who fills more team needs
 * still wins: "a level 60 is still better than a level 50, but not if the level 50 fills more needs."
 *
 * FOUR SUNK-INVESTMENT AXES, multiplicative (2026-08-05, Mike): level/stars ALONE compressed the
 * spread far too much — "Mikey L60 + masteries + books vs Mausoleum Mage L40, no books, no masteries
 * are nowhere near equal; he MIGHT be worth HALF." The old level-only, floored-at-0.5 curve put that
 * pair at 0.833 vs 1.0 (a 17% gap) — wrong. Masteries (Warmaster/Giant Slayer are a LARGE share of a
 * champ's damage, especially on CB) and books (cooldowns + debuff chances) are real permanent power the
 * flat curve was BLIND to. We now read all four — level, stars, masteries, books — from the same Gestal
 * signal that already feeds the roster (has_boss_mastery / mastery_tier / is_booked, lib/gestal-context).
 *
 * CAPTURE-GAP DISCIPLINE (unchanged convention): a MISSING field ⇒ neutral 1.0 (never a penalty for
 * un-captured data). Only an EXPLICIT false penalises — and Gestal always sets these booleans, so the
 * penalty is real where the data is real and silent where it isn't (test fixtures carry no mastery/book
 * field and are unaffected). GEAR still EXCLUDED (portable; see landRate / gear-ceiling layer).
 *
 * SHAPE: raw = levelStars × mastery × book, floored at BUILD_MIN so a genuine but unbuilt champ still
 * DELIVERS (bias, NEVER a veto — the repair loop can still field them for a unique need). Landmarks:
 * L60/6★ + masteries + books → 1.0; L50/5★ maxed-for-rank + both → ~0.85; **L40/4★, no books, no
 * masteries → ~0.54** (Mike's "about half" anchor); raw fodder → the floor.
 */
export const BUILD_MIN = 0.12;
export function buildScale(champ) {
  const lvl = champ?.level, str = champ?.stars;
  if (lvl == null && str == null) return 1;                // capture gap ⇒ 1.0, never a penalty
  const ls = 0.60 * Math.min(1, (lvl ?? 0) / 60)
           + 0.40 * Math.min(1, (str ?? 0) / 6);
  // MASTERIES — boss mastery (Warmaster/Giant Slayer) is the load-bearing one; a full non-boss tree
  // still helps. undefined (no capture) ⇒ 1.0; explicit "no masteries" ⇒ 0.80.
  const bm = champ?.has_boss_mastery, mt = champ?.mastery_tier;
  const m = bm === true            ? 1.0
          : (bm === undefined && mt === undefined) ? 1.0   // capture gap
          : mt === 'Complete'      ? 0.90                  // full tree, no boss mastery
          :                          0.80;                 // none
  // BOOKS — shorter cooldowns + higher debuff chances. GRADED by actual progress where we have it
  // (book_fraction): a champ one book short (Michelangelo 3/4 ≈ 0.92) barely dips; a bare champ (Ninja
  // 1/x ≈ 0.19) sits near the floor. Penalty spans [0.90 … 1.0] across [0% … 100%] booked. Falls back
  // to the binary is_booked when only that is present; undefined ⇒ 1.0 (capture gap).
  const bf = champ?.book_fraction;
  const b = typeof bf === 'number' ? 0.90 + 0.10 * Math.max(0, Math.min(1, bf))
          : champ?.is_booked === undefined ? 1.0
          : champ?.is_booked ? 1.0 : 0.90;
  const raw = ls * m * b;
  return BUILD_MIN + (1 - BUILD_MIN) * raw;
}
...
export function tagDelivery(tag, champ, skillRows, tagMeta,
  ...
  // AFFINITY placement is a fixed CHAMPION property (a weak-affinity champ Weak-Hits regardless of
  // gear), so it stays in `delivery` — it is a real, unchangeable selection factor.
  const af = affinityPlacementFactor(tag, champ, tagMeta, { bossAffinity, skillRows });
  // BUILD SCALE = sunk investment (level/stars). Level IS a selection criterion (pick level-ready
  // champions), so this stays in `delivery`. bs≤1, so it can only shrink delivery, never inflate.
  const bs = buildScale(champ);
  const matchup = bossAffinity ? affinityMatchup(champ.affinity, bossAffinity) : null;
  return {
    uptime: up, chance: ch, land: lr, affinity: af, buildScale: bs,
    ...
    // landRate (ACC vs floor) is DELIBERATELY NOT a factor in `delivery` (2026-08-01, Mike): SELECTION
    // picks by KIT + level, NOT gear. Gear is PORTABLE, so "will this debuff LAND?" is a CEILING
    // question — can the gear pool gear this seat to the ACC floor for the target stage? — answered in
    // the (separate) gear-ceiling layer, never a selection multiplier. Folding ACC into delivery buried
    // whole kits on gear the model reads wrong anyway (Artak's HP Burn at ACC 30→land 0.15, real 175).
    // `land` is STILL returned as a standalone gate flag (→ GEAR THIS SEAT / ceiling), so the ACC need
    // is surfaced as an ACTION instead of silently docking the champion. (Was: up*ch*lr*af*bs — INS-0031's
    // landRate discrimination now belongs to the ceiling, not the pick.)
    delivery: up * ch * af * bs,
  };
}
