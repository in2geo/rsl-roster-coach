...
// `Decrease Attack` added 2026-08-01 (Mike): on Spider it is real SURVIVAL — halving the swarm's (and
// Skavag's) ATK cuts incoming across the whole team (Artak's A2 50% AoE Decrease ATK is why his teams
// barely take damage). Weighted below direct sustain (0.6): it MITIGATES incoming, it doesn't RECOVER HP.
const SURVIVE = { 'Healer': 1.0, 'AoE Heal': 1.0, 'Continuous Heal': 1.0, 'AoE Shield': 1.0,
                  ...
                  'Decrease Attack': 0.6, 'Leech': 0.35 };

// DETONATION for the HP-BURN path — the anti-consume DETONATOR (Mike, 2026-08-01). `Debuff Activation`
// force-ticks all [HP Burn] immediately (Artak's A2), so burning Spiderlings DIE before Skavag's turn
// and cannot be consumed — that is the Spider win condition, and it was scored ZERO. Analogous to
// poison_explosion's `detonation`, but for HP Burn. (Not a placer — it CASHES IN burn already there.)
const DETONATION_HPBURN = { 'Debuff Activation': 1.0 };
...
export const SPIDER_STRATEGIES = [
  ...
  { key: 'hp_burn', name: 'AoE HP Burn — %MaxHP damage over time', stages: [15, 25],
    // 2026-08-01 (Mike): +detonation (Debuff Activation = the anti-consume detonator), sourced from
    // crowd_control (20→10 — the header already flags CC as SURVIVE-work, not anti-heal). `coverage:
    // probabilistic` makes MULTIPLE HP-Burn appliers score as reliable UPTIME (1−Π(1−rel)), not 0.30
    // surplus — a 3-applier burn team (Hilvi+Artak+Ninja) is the reliable engine, not redundancy.
    allocation: { hp_burn: 30, detonation: 10, crowd_control: 10, boss_damage: 15, survive: 15, tempo: 10, tm_lock: 10 },
    buckets: { hp_burn: HP_BURN, detonation: DETONATION_HPBURN, crowd_control: CROWD_CONTROL,
               boss_damage: BOSS_DAMAGE, survive: SURVIVE, tempo: TEMPO, tm_lock: TM_LOCK },
    coverage: { hp_burn: 'probabilistic' }, ...COMMON },
];
