...
export const SPIDER_STRATEGIES = [
  ...
  { key: 'maxhp_nuke', name: 'Enemy MAX HP nuke — the wall (AoE stops working)', stages: [15, 20],
  ...
  { key: 'hp_burn', name: 'AoE HP Burn — mandatory 21-25 (%MaxHP capped)', stages: [15, 25],
...
/** Strategies viable at a given stage. The stage GATES; the model picks among what's left. */
export const spiderStrategiesForStage = stage =>
  SPIDER_STRATEGIES.filter(s => stage >= s.stages[0] && stage <= s.stages[1]);
