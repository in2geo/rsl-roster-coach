...
/** Does a champ satisfy this need (all tags of ANY one solution)? */
function championCovers(champ, need) {
  const tags = new Set(champ.tags ?? []);
  return need.solutions.some(sol => sol.every(t => tags.has(t)));
}
 ...
 */
function marginalValue(champ, team, needs, roleCoverCount, contentKey) {
  let v = 0;
  for (const need of needs) {
    if (!championCovers(champ, need)) continue;
    v += need.weight * saturationFactor(need.role, roleCoverCount[need.role] ?? 0);
  }
  return v + 0.15 * magnitude(champ, contentKey); // magnitude is a tiebreaker, not the driver
}
 ...
 */
export function constructTeam(roster = [], needs = [], { contentKey, size = 5, eligible = () => true } = {}) {
  ...
  while (team.length < size && pool.length) {
    ...
    for (const c of pool) {
      ...
      const v = marginalValue(c, team, needs, roleCoverCount, contentKey);
...
function needStrength(champ, need, contentKey) {
  if (need.role === 'wave_or_add') {
    const aoeDmg = (champ.tags ?? []).includes('AoE Damage') ? 0.4 : 0;
    return controlStrength(champ.tags ?? []) + aoeDmg; // AoE-CC lockdown + AoE clear
  }
  if (need.role === 'boss_damage') return magnitude(champ, contentKey);
  return 0.5; // sustain/debuff/turn — presence is what matters, not magnitude
}
 ...
 */
export function potentialBuilds(roster = [], needs = [], builtTeam = [], { isBuilt = () => true, contentKey, upgradeMargin = 1.3 } = {}) {
  // Built team's best role-strength per need.
  const builtBest = needs.map(need => Math.max(0, ...builtTeam.filter(c => championCovers(c, need)).map(c => needStrength(c, need, contentKey))));
  const out = [];
  for (const c of roster) {
    ...
    needs.forEach((need, i) => {
      ...
      const s = needStrength(c, need, contentKey);
