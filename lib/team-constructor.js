...
function marginalValue(champ, team, needs, roleCoverCount, opts) {
  ...
  return v + 0.15 * magnitude(champ, opts.contentKey); // magnitude is a tiebreaker, not the driver
}
 ...
 */
export function constructTeam(roster = [], needs = [], { contentKey, size = 5, eligible = () => true, tagMeta = {}, accFloor = null } = {}) {
  const opts = { contentKey, tagMeta, accFloor }; // built team scored at CURRENT reliability
