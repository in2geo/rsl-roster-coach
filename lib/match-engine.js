...
//
// borderline threshold: passes but estimated value is within 10% above the floor —
// close enough that real substat variance could tip it either way.
function computeVerdictBand(gaps, thresholdResults, team) {
  if (gaps.length >= 2) return { verdict_band: 'multi_goal_gap',        confidence_pct: 12 };
  if (gaps.length === 1) return { verdict_band: 'one_goal_gap',          confidence_pct: 30 };
  ...
  if (failing.length)    return { verdict_band: 'stats_failing',          confidence_pct: 47 };
  if (borderline.length) return { verdict_band: 'borderline_threshold',   confidence_pct: 62 };

  const minGear = Math.min(...(team ?? []).map(c => GEAR_WEIGHT[c.gear_tier] ?? 1));
  if (minGear >= 3)      return { verdict_band: 'all_goals_strong_gear',  confidence_pct: 90 };
                         return { verdict_band: 'all_goals_dungeon_gear', confidence_pct: 77 };
}
...
export async function matchRoster(roster, contentKey, options = {}) {
  ...
  const coveredGoals = totalGoals - gaps.length;
  const verdict = totalGoals === 0
    ? null
    : gaps.length === 0
      ? 'ready'
      : coveredGoals >= totalGoals / 2
        ? 'borderline'
        : 'not_ready';
