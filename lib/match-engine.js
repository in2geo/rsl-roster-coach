...
import { estimateStats, masteryDamageModifier } from './estimate-stats.js';
...
function selectTeam(mapped, goals, coverage) {
  ...
  scored.sort((a, b) =>
    b.score - a.score ||
    // Phase 5: among equal tag-coverage, prefer the stronger role-relevant damage
    // skill (champion_skills multiplier). null (Support / no data) sorts last.
...
export async function matchRoster(roster, contentKey, options = {}) {
  ...
  const stun_matrix = contentKey === 'clan_boss'
    ...
    : null;
  // Clan Boss only: account masteries scale damage output (not stats). Surfaced for
  // the explanation/UI; applies once a CB damage estimate consumes it.
  const mastery_damage_modifier = contentKey === 'clan_boss'
    ? masteryDamageModifier(options.masteries) : null;
