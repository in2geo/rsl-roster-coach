...
function computeVerdictBand(gaps, thresholdResults, team) {
  const band = (() => {
    ...
    // SOFT floors (ACC — a debuff land-chance, not a survival gate) never force a
    // failing band; they degrade confidence multiplicatively below (floors-are-not-gates).
    // Only HARD thresholds (HP/DEF/SPD/RES survival/turn-order floors) hard-fail a stage.
    const failing    = thresholdResults.filter(t => t.passes === false && !t.soft);
  ...
  // ACC is a SOFT floor: a carrier below the stage's ACC floor still lands its debuff,
  // just less reliably, so the clear is slower — not a loss. Degrade confidence in
  // proportion to how far below the floor the weakest ACC-dependent debuff sits, rather
  // than hard-capping the stage at the floor (which lowballed Spider badly). The 0.55
  // ceiling on the penalty keeps a far-below-floor stage recommendable-but-low, never 0.
  const accSoft = thresholdResults.filter(t => t.soft && t.acc_reliability != null && t.acc_reliability < 1);
  if (accSoft.length) {
    const rel = Math.min(...accSoft.map(t => t.acc_reliability));
