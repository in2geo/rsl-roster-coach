...
function computeVerdictBand(gaps, thresholdResults, team) {
  const band = (() => {
    ...
    /* NO STAT FLOOR SELECTS A BAND ANY MORE (Mike, 2026-07-20 — floors are RECOMMENDATIONS).
     * Every threshold is soft and degrades confidence continuously below; see evaluateThresholds.
     * The band is decided by COVERAGE GAPS and gear only.
     *
     * WHY GAPS STAY HARD while floors do not: a gap means NO owned champion supplies the
     * capability at all — the team cannot perform the mechanic by any amount of grinding. That is
     * a real absence, not a shortfall, and CLAUDE.md requires it be surfaced rather than guessed
     * around. A stat below its floor is the opposite: the champion does the thing, just less
     * reliably. One is "you don't have a cleanser"; the other is "your cleanser is slow".
     *
     * `stats_failing` / `borderline_threshold` remain in VERDICT_BAND_CONFIG deliberately — the
     * bands are referenced by STRATEGY.md and by the Phase-2 calibration query, and historical
     * run_reconciliations rows carry them. They are simply no longer reachable from thresholds. */
  ...
  // Soft floors are excluded here; they apply their own multiplier below.
  ...
  /* EVERY stat floor degrades confidence continuously — none of them gates.
   * Below a floor the team still plays: a debuff lands less often, or the boss's debuffs land on
   * you more often, or you take more hits. All of that shows up as a SLOWER, riskier clear — the
   * thing `CLAUDE.md` says we judge by (TIME) — never as "you may not attempt this stage".
   *
   * WEAKEST LINK, not a product. The worst shortfall drives the penalty rather than compounding
   * every stat, because (a) it mirrors how the ACC term already behaved, and (b) with the declared
   * floors measurably 2-3x too high (max RES across six accounts is 119 against floors of 250-300),
   * compounding three inflated shortfalls would re-create the under-recommendation from the other
   * direction. The floors themselves still need recalibrating against outcomes; this changes only
   * whether they GATE.
   *
   * The 0.55 ceiling keeps a far-below-floor stage recommendable-but-low, never 0 — you can always
   * attempt it. Both magnitudes (0.55 floor, linear ramp) are NOMINAL and uncalibrated. */
  ...
  if (softs.length) {
    ...
    confidence_pct *= 0.55 + 0.45 * Math.max(0, Math.min(1, rel));
  }
...
async function scanSpiderStages(mapped, dungeonId) {
  ...
  for (const group of SPIDER_SCAN_GROUPS) {
    ...
    for (const stageNumber of group.stageNumbers) {
      ...
      lastResult = { stageNumber, dungeon_stage_id: stageRow.id, stageLabel: group.label, goals, actionableGoals, gaps, coverage, threshold_results, verdict_band, confidence_pct, affinity, data_warning, team, leader, synergies: detectSynergies(team) };
...
async function scanDungeonStages(mapped, dungeonId, contentKey) {
  ...
  for (const stageRow of stages ?? []) {
    ...
    lastResult = { stageNumber: stageRow.stage_number, dungeon_stage_id: stageRow.id, stageLabel: stageRow.label, goals, actionableGoals, gaps, coverage, threshold_results, verdict_band, confidence_pct, affinity, data_warning, team, leader, phases: phases.map(p => p.phase_type), synergies: detectSynergies(team) };
...
export async function matchRoster(roster, contentKey, options = {}) {
  ...
  // Runs alongside coverage: per-champion contribution grounded in the damage-mechanics
  // interaction rules (source-split output, debuff multipliers weighted by reliability +
  // saturation, mismatch → ~0, multiplicative sustain), plus a two-sided confidence. It is
  // NON-AUTHORITATIVE until the Layer 3 gate is met (PROJECT_BRIEF §5b): magnitudes are
  // nominal, boss HP is real only for CB (nominal elsewhere), and the survival side needs
  // incoming-damage-per-stage (a known data gap), so it is flagged `estimated`.
  const NOMINAL_BOSS_HP = 15_000_000; // placeholder so %maxHP-vs-attack mix registers off-CB
  let contribution = null;
  {
    ...
    const cResult = computeContributions(contribTeam, {
      bossHp: contribBossHp ?? NOMINAL_BOSS_HP,
      incomingDamagePerTurn: null, // known gap → survival side estimated, flagged below
    });
    contribution = {
      estimated: true,
      boss_hp_source: contribBossHp ? 'clan_boss_stats' : 'nominal',
      ...
      note: 'Directional only (nominal magnitudes; survival needs incoming-damage-per-stage). ' +
