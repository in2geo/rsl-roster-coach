...
// Evaluates formula-based and relative stat threshold checks for a given stage number.
function evaluateThresholds(thresholdChecks, stageNumber, mapped) {
  const results = [];
...
function computeTagCoverage(goals, rosterTagSet) {
  ...
  for (const goal of goals ?? []) {
    ...
    coverage[goal.id] = { description: goal.description, satisfied, solution_label: matchedLabel, total_solutions: solutions.length };
...
async function scanSpiderStages(mapped, dungeonId) {
  ...
  for (const group of SPIDER_SCAN_GROUPS) {
    ...
    for (const stageNumber of group.stageNumbers) {
      ...
      lastResult = { stageNumber, dungeon_stage_id: stageRow.id, stageLabel: group.label, goals, actionableGoals, gaps, coverage, threshold_results, verdict_band, confidence_pct, data_warning };
...
export async function matchRoster(roster, contentKey, options = {}) {
  ...
  const contentConfig = (isEventDungeon || isSpider) ? null : CONTENT_MAP[contentKey];
  if (!isEventDungeon && !isSpider && !contentConfig) throw new Error(`Unknown content key: ${contentKey}`);

  // Clan Boss: difficulty is passed at runtime (Normal, Hard, Brutal, etc.)
  // For event dungeons these are resolved from the DB below.
  let dungeonName  = contentConfig?.dungeon ?? null;
  ...
  if (isSpider) {
    ...
    const team = selectTeam(mapped, scan.goals, scan.coverage);
...
export async function evaluateTeam(rosterTeam, dungeonName, stageNumber, difficulty = null) {
  ...
  if (!dungeon) return { seeded: false, reason: `Dungeon "${dungeonName}" not in DB` };

  const { data: stages } = await supabase
    .from('dungeon_stages').select('id, label, stage_number').eq('dungeon_id', dungeon.id);

  // Candidate stages matching the number (exact stage_number, or the number inside a
  // grouped label like "Stages 7-10").
  ...
  if (!candidates.length) return { seeded: false, reason: `${dungeonName} stage ${stageNumber} not seeded (out of scope)` };

  // Disambiguate by difficulty (it's in the label). A known difficulty must match —
  // never fall back to the other difficulty, since Hard requirements ≠ Normal. Only
  // when difficulty is unknown (null) do we guess (prefer the non-Hard variant).
  ...
  if (!stage) return { seeded: false, reason: `${dungeonName} ${difficulty} stage ${stageNumber} seeded only at the other difficulty` };
  ...
  return {
    seeded: true,
    dungeon: dungeonName, stage_label: stage.label, stage_number: stageNumber, difficulty: difficulty ?? null,
