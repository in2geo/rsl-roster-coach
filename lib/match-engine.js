...
// Stages 11-20 reuse the "Stages 7-10" coverage row: the Spider mechanic (spiderlings + boss
// single-target hits) is identical across stages, and the only thing that scales is the stat
// floor — the ACC threshold is `stage * 10`, evaluated per stageNumber, so it tightens correctly
// as the scan climbs. Without this the scan hard-capped at Stage 10 (captured data shows this
// audience clears well past it). Content_label uses the stageNumber, so it still reads "Stage 17".
const SPIDER_SCAN_GROUPS = [
  { label: 'Stages 7-10', stageNumbers: [20, 19, 18, 17, 16, 15, 14, 13, 12, 11, 10, 9, 8, 7] },
...
function gearTierWeight(t) {
  return { starter: 1, fair: 2, good: 3, endgame: 4 }[String(t ?? '').toLowerCase()] ?? 1;
}
...
export function mapRoster(roster, opts = {}) {
  ...
  const mapped = (roster ?? []).map(uc => {
    ...
    return {
      ...
      // Gear tier is account-level (from the profile); every champion shares it.
      // Normalize casing so GEAR_WEIGHT / gearTierWeight lookups are UI-casing-proof.
      gear_tier: String(gearTier ?? uc.gear_tier ?? 'starter').toLowerCase(), ascension_level: uc.ascension_level,
      tags: approvedTags,
...
function usabilityTier(champ) {
  ...
  if (lvl >= 50) return 3; // effectively built
