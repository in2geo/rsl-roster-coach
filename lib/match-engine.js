...
export function mapRoster(roster, opts = {}) {
  ...
  const mapped = (roster ?? []).map(uc => {
    ...
    return {
      ...
      // Full decoded mastery NAMES (Gestal rosters only; [] for manual). Inert for the product's tag/select
      // path — carried so the SIMULATOR (allyCombatant → makeCombatant → deriveMasteryMech) can model the
      // target-type-aware boss bonus + offense-proc damage masteries, not just the has_boss_mastery boolean.
      masteries: uc.masteries ?? [],
    };
  });
