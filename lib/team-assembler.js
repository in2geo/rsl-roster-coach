...
// Identity here is name-keyed: the caller builds champ objects as { name, caps, dev } with NO
// `champions.id` (see tools/assemble-team.mjs), and team/roster names all originate from the SAME
// account snapshot, so they're internally consistent. Route those comparisons through the shared
// registry normalizer (normalizeName) so this file uses the ONE app-wide normalizer rather than raw
// `.name ===`. `.name` is still carried verbatim into the swaps output for display.
...
export function assembleTeam(champs, roleDefs, order, maxSeats = 5) {
  ...
  while (uncovered.size && team.length < maxSeats) {
    ...
    for (const c of cand) {
      if (chosen.has(normalizeName(c.name))) continue;
    ...
    chosen.add(normalizeName(best.name));
...
export function fixTeam(teamChamps, shortRole, roster, roleDefs, order) {
  ...
  const teamNames = new Set(team.map(c => normalizeName(c.name)));
  const originalRoles = new Set(team.flatMap(c => c.roles));
  const incoming = roster.filter(c => !teamNames.has(normalizeName(c.name)))
  ...
  for (const seat of team) {
    const others = team.filter(x => normalizeName(x.name) !== normalizeName(seat.name));
