...
// Clan-Boss damage output modifier from account-wide masteries (Warmaster / etc.).
// Applied as a multiplier on damage calcs, NOT to stats. Full masteries ≈ +15%.
...
export function estimateStats(champion, userChampion, opts = {}) {
  ...
  const acct = ACCOUNT_DEV[devOf(opts)];
  return {
    hp:  Math.round((champion.base_hp  ?? 0) * (1 + gear.hp  + acct.hp)),
    atk: Math.round((champion.base_atk ?? 0) * (1 + gear.atk + acct.atk)),
    def: Math.round((champion.base_def ?? 0) * (1 + gear.def + acct.def)),
