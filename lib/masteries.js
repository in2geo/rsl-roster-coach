...
// modest flat/utility effect, negligible next to a %-of-boss-HP source. Both require a level-60
// champion (tier-6 unlock), so the manual input only needs to ask about level-60 champions.
//
...
export function resolveBossMastery(champ) {
  ...
  if ((champ?.level ?? 0) < 60) return false;
