 ...
 */
export function championPerTurnBySource(champ, { bossHp = 0, maxHpCap = null } = {}) {
  const out = {};
  ...
  for (const tag of (champ.tags ?? [])) {
    ...
    if (src === 'attack') {
      ...
      /* %maxHP source: fraction × bossHp, damped by how saturated that source is on the team.
       *
       * For `enemy_maxhp`, prefer the REAL per-skill percentage extracted from verbatim skill
       * text (seed 202) over the flat nominal. That is what makes the §6b boss cap MEAN anything:
       * the nominal 0.05 sits under the 0.10 ceiling so the cap could never bind, whereas the real
       * values run 0.01–0.30 and three of the twelve genuinely exceed it. `maxhp_damage` already
       * carries the skill's own boss value and cap folded in (see maxHpDamageProfile); the CONTENT
       * cap is applied here, last, because it is a property of the stage rather than the skill.
       *
       * The cap clamps the FRACTION, never the product — it is "at most 10% of the boss's max HP
       * per hit", so applying it after multiplying by bossHp would be a different (and wrong) rule. */
      const nominal = src === 'enemy_maxhp' && champ.maxhp_damage?.pct != null
        ? Math.min(champ.maxhp_damage.boss_pct ?? champ.maxhp_damage.pct,
                   champ.maxhp_damage.skill_cap ?? Infinity)
        : (SOURCE_COEFF[src] ?? 0);
      const frac = applyMaxHpCap(src, nominal, maxHpCap);
      add(src, frac * bossHp * saturationValue(src, champ._activeStacks?.[src] ?? 0));
    }
  }
  if (champ.has_boss_mastery) {
    add('warmaster', applyMaxHpCap('warmaster', SOURCE_COEFF.warmaster ?? 0, maxHpCap) * bossHp);
  }
  return out;
}
...
export function computeContributions(team = [], opts = {}) {
  const {
    ...
    // Boss-content %maxHP ceiling (damage-mechanics §6b) — from maxHpCapFor({stageNumber,…}).
    // null = uncapped. Scoped to `enemy_maxhp`; DoT/Warmaster capping is UNVERIFIED.
    maxHpCap = null,
  } = opts;
  ...
  const rows = team.map(c => {
    ...
    const perTurn = championPerTurnBySource(c, { bossHp, maxHpCap });
    const bySource = {};
    let ownDamage = 0;
  ...
  // ONE identity index for crediting granted damage/survival back to owners. contribTeam
  // objects carry no `.id`, and owners are internal team `.name`s, so key by the shared
  // registry normalizer (normalizeName) rather than a raw `r.name === owner` (lib/champion-names.js).
  const rowByName = new Map(rows.map(r => [normalizeName(r.name), r]));
  ...
  for (const s of SOURCES) {
    ...
    for (const a of attribution) {
      const owner = rowByName.get(normalizeName(a.owner));
  ...
  if (turn.lift > 0 && teamDamage > 0) {
    ...
    for (const [tag, owners] of Object.entries(turn.providers)) {
      ...
      for (const owner of owners) {
        const row = rowByName.get(normalizeName(owner));
  ...
  if (incomingDamagePerTurn != null && incomingDamagePerTurn > 0) {
    ...
    for (const c of team) {
      ...
      const row = rowByName.get(normalizeName(c.name));
