...
export function championPerTurnBySource(champ, { bossHp = 0, maxHpCap = null } = {}) {
  ...
  for (const tag of (champ.tags ?? [])) {
    ...
    if (src === 'attack') {
      ...
      // and CLAMPED by the boss-content cap (damage-mechanics §6b) before it meets bossHp — the cap
      // is a ceiling on the FRACTION, so it must apply to the coefficient, not the product.
      const frac = applyMaxHpCap(src, SOURCE_COEFF[src] ?? 0, maxHpCap);
